<?php
defined('BASEPATH') OR exit('No direct script access allowed');

$route['system/feature-access'] = 'feature_access/upgrade';

$route['default_controller'] = 'auth';
$route['404_override'] = '';
$route['translate_uri_dashes'] = FALSE;

// Auth
$route['login']  = 'auth/index';
$route['logout'] = 'auth/logout';

// Dashboard
$route['dashboard'] = 'dashboard/index';
$route['dashboard/production-suggestions'] = 'dashboard/production_suggestions';

// Settings
$route['settings']                  = 'settings/index';
$route['settings/change-password']  = 'settings/change_password';

// Read-only customer handbook; server chapters have a separate permission.
$route['guide'] = 'user_guide/index';

// Attendance
$route['attendance/settings'] = 'attendance/settings';
$route['attendance/daily'] = 'attendance/daily';
$route['attendance/logs'] = 'attendance/logs';
$route['attendance/schedules'] = 'attendance/schedules';
$route['attendance/schedules-v2'] = 'attendance/schedules_v2';
$route['attendance/schedule-v2'] = 'attendance/schedules_v2';
$route['attendance/schedules-v2/save'] = 'attendance/schedule_v2_save';
$route['attendance/schedules/store'] = 'attendance/schedule_store';
$route['attendance/schedules/update/(:num)'] = 'attendance/schedule_update/$1';
$route['attendance/schedules/delete/(:num)'] = 'attendance/schedule_delete/$1';
$route['attendance/schedules/bulk-store'] = 'attendance/schedule_bulk_store';
$route['attendance/pending-requests'] = 'attendance/pending_requests';
$route['attendance/pending-requests/action/(:num)'] = 'attendance/pending_request_action/$1';
$route['attendance/pending-requests/bulk-action'] = 'attendance/pending_request_bulk_action';
$route['attendance/overtime-entries'] = 'attendance/overtime_entries';
$route['attendance/overtime-entries/store'] = 'attendance/overtime_entry_store';
$route['attendance/overtime-entries/update/(:num)'] = 'attendance/overtime_entry_update/$1';
$route['attendance/overtime-entries/delete/(:num)'] = 'attendance/overtime_entry_delete/$1';
$route['attendance/anomalies'] = 'attendance/anomalies';
$route['attendance/master-health'] = 'attendance/master_health';
$route['attendance/estimate'] = 'attendance/estimate';
$route['attendance/estimate/detail/(:num)'] = 'attendance/estimate_detail/$1';
$route['attendance/meal-calendar'] = 'attendance/meal_calendar';
$route['attendance/ph-assignments'] = 'attendance/ph_assignments';
$route['attendance/ph-assignments/save'] = 'attendance/ph_assignment_save';
$route['attendance/ph-assignments/delete/(:num)'] = 'attendance/ph_assignment_delete/$1';
$route['attendance/ph-ledger'] = 'attendance/ph_ledger';
$route['attendance/ph-ledger/store'] = 'attendance/ph_ledger_store';
$route['attendance/ph-ledger/update/(:num)'] = 'attendance/ph_ledger_update/$1';
$route['attendance/ph-ledger/delete/(:num)'] = 'attendance/ph_ledger_delete/$1';
$route['attendance/ph-ledger/sync-grants'] = 'attendance/ph_ledger_sync_grants';
$route['attendance/ph-recap'] = 'attendance/ph_recap';

// HR Contract Operational
$route['hr/contracts'] = 'hr_contracts/index';
$route['hr/contracts/create-draft'] = 'hr_contracts/create_draft';
$route['hr/contracts/(:num)'] = 'hr_contracts/view/$1';
$route['hr/contracts/(:num)/generate'] = 'hr_contracts/generate_contract/$1';
$route['hr/contracts/(:num)/approve'] = 'hr_contracts/approve/$1';
$route['hr/contracts/(:num)/sign'] = 'hr_contracts/sign/$1';
$route['hr/contracts/(:num)/transition/(:any)'] = 'hr_contracts/transition/$1/$2';

// HR Contract compatibility routes (core-like)
$route['hr-contracts'] = 'hr_contracts/index';
$route['hr-contracts/templates'] = 'hr_contracts/templates';
$route['hr-contracts/template-edit'] = 'hr_contracts/template_edit';
$route['hr-contracts/template-edit/(:num)'] = 'hr_contracts/template_edit/$1';
$route['hr-contracts/template-delete/(:num)'] = 'hr_contracts/template_delete/$1';
$route['hr-contracts/template-preview'] = 'hr_contracts/template_preview';
$route['hr-contracts/generate'] = 'hr_contracts/generate';
$route['hr-contracts/view/(:num)'] = 'hr_contracts/view/$1';
$route['hr-contracts/print/(:num)'] = 'hr_contracts/print_view/$1';
$route['hr-contracts/(:num)/approve'] = 'hr_contracts/approve/$1';
$route['hr-contracts/(:num)/sign'] = 'hr_contracts/sign/$1';
$route['hr-contracts/(:num)/transition/(:any)'] = 'hr_contracts/transition/$1/$2';
$route['hr-contracts/verify/(:any)'] = 'hr_contract_verify/index/$1';
$route['payroll/preview-thp'] = 'payroll/preview_thp';
$route['payroll/manual-adjustments'] = 'payroll/manual_adjustments';
$route['payroll/manual-adjustments/store'] = 'payroll/manual_adjustment_store';
$route['payroll/manual-adjustments/update/(:num)'] = 'payroll/manual_adjustment_update/$1';
$route['payroll/manual-adjustments/delete/(:num)'] = 'payroll/manual_adjustment_delete/$1';
$route['payroll/meal-disbursements'] = 'payroll/meal_disbursements';
$route['payroll/meal-disbursements/generate'] = 'payroll/meal_disbursement_generate';
$route['payroll/meal-disbursements/mark-paid/(:num)'] = 'payroll/meal_disbursement_mark_paid/$1';
$route['payroll/meal-disbursements/void/(:num)'] = 'payroll/meal_disbursement_void/$1';
$route['payroll/salary-disbursements'] = 'payroll/salary_disbursements';
$route['payroll/payroll-periods'] = 'payroll/payroll_periods';
$route['payroll/salary-disbursements/period-generate'] = 'payroll/payroll_period_generate';
$route['payroll/salary-disbursements/period-void/(:num)'] = 'payroll/payroll_period_void/$1';
$route['payroll/salary-disbursements/period-delete/(:num)'] = 'payroll/payroll_period_delete/$1';
$route['payroll/salary-disbursements/generate'] = 'payroll/salary_disbursement_generate';
$route['payroll/salary-disbursements/mark-paid/(:num)'] = 'payroll/salary_disbursement_mark_paid/$1';
$route['payroll/salary-disbursements/slip/(:num)'] = 'payroll/salary_disbursement_slip/$1';
$route['payroll/salary-disbursements/void/(:num)'] = 'payroll/salary_disbursement_void/$1';
$route['payroll/salary-disbursements/delete/(:num)'] = 'payroll/salary_disbursement_delete/$1';
$route['payroll/cash-advances'] = 'payroll/cash_advances';
$route['payroll/cash-advances/store'] = 'payroll/cash_advance_store';
$route['payroll/cash-advances/update/(:num)'] = 'payroll/cash_advance_update/$1';
$route['payroll/cash-advances/pay-installment/(:num)'] = 'payroll/cash_advance_pay_installment/$1';
$route['payroll/cash-advances/void/(:num)'] = 'payroll/cash_advance_void/$1';
$route['payroll/cash-advances/delete/(:num)'] = 'payroll/cash_advance_delete/$1';
$route['payroll/bonus'] = 'payroll/bonus';
$route['payroll/bonus/generate-pool'] = 'payroll/bonus_pool_generate';
$route['payroll/bonus/sync-auto-penalties'] = 'payroll/bonus_auto_penalty_sync';
$route['payroll/bonus/approve-pool/(:num)'] = 'payroll/bonus_pool_approve/$1';
$route['payroll/bonus/void-pool/(:num)'] = 'payroll/bonus_pool_void/$1';
$route['payroll/bonus/delete-pool/(:num)'] = 'payroll/bonus_pool_delete/$1';
$route['payroll/bonus/delete-pool-bulk'] = 'payroll/bonus_pool_bulk_delete';
$route['payroll/bonus/monthly-detail/(:num)/(:num)'] = 'payroll/bonus_monthly_detail/$1/$2';
$route['payroll/bonus/daily-detail/(:num)'] = 'payroll/bonus_daily_detail/$1';
$route['payroll/bonus/generate-service-metric'] = 'payroll/bonus_service_metric_generate';
$route['payroll/bonus/service-metric-generate-month'] = 'payroll/bonus_service_metric_generate_month';
$route['payroll/bonus/generate-monthly-summary'] = 'payroll/bonus_monthly_summary_generate';
$route['payroll/bonus/config-save'] = 'payroll/bonus_config_save';
$route['payroll/bonus/config-delete/(:num)'] = 'payroll/bonus_config_delete/$1';
$route['payroll/bonus/rule-save'] = 'payroll/bonus_rule_save';
$route['payroll/bonus/rule-delete/(:num)'] = 'payroll/bonus_rule_delete/$1';
$route['payroll/bonus/weight-save'] = 'payroll/bonus_weight_save';
$route['payroll/bonus/weight-delete/(:num)'] = 'payroll/bonus_weight_delete/$1';
$route['payroll/bonus/penalty-save'] = 'payroll/bonus_penalty_type_save';
$route['payroll/bonus/penalty-delete/(:num)'] = 'payroll/bonus_penalty_type_delete/$1';
$route['payroll/bonus/penalty-event-save'] = 'payroll/bonus_penalty_event_save';
$route['payroll/bonus/penalty-event-void/(:num)'] = 'payroll/bonus_penalty_event_void/$1';
$route['payroll/bonus/peer-moderate/(:num)'] = 'payroll/bonus_peer_moderate/$1';

// Employee Portal
$route['my'] = 'my/index';
$route['my/attendance'] = 'my/attendance';
$route['my/attendance/mark'] = 'my/attendance_mark';
$route['my/profile'] = 'my/profile';
$route['my/profile/contracts/sign/(:num)'] = 'my/profile_contract_sign/$1';
$route['my/profile/contracts/print/(:num)'] = 'my/profile_contract_print/$1';
$route['my/schedule'] = 'my/schedule';
$route['my/payroll'] = 'my/payroll';
$route['my/payroll-slip/(:num)'] = 'my/payroll_slip/$1';
$route['my/leave-requests'] = 'my/leave_requests';
$route['my/leave-requests/schedule'] = 'my/leave_request_schedule';
$route['my/leave-requests/cancel/(:num)'] = 'my/leave_request_cancel/$1';
$route['my/meal-ledger'] = 'my/meal_ledger';
$route['my/overtime'] = 'my/overtime';
$route['my/ph-ledger'] = 'my/ph_ledger';
$route['my/manual-adjustments'] = 'my/manual_adjustments';
$route['my/cash-advance'] = 'my/cash_advance';
$route['my/bonus'] = 'my/bonus';
$route['my/bonus/daily-detail/(:num)'] = 'my/bonus_daily_detail/$1';
$route['my/bonus/peer-submit'] = 'my/bonus_peer_submit';

// Inventory flow (item -> material)

// Purchase
$route['procurement'] = 'procurement/store_requests';
$route['procurement/division-requests'] = 'procurement/division_requests';
$route['procurement/purchasing-desk'] = 'procurement/purchasing_desk';
$route['store-requests'] = 'procurement/store_requests';
$route['store-requests/create'] = 'procurement/store_request_create';
$route['store-requests/detail/(:num)'] = 'procurement/store_request_detail/$1';
$route['store-requests/edit/(:num)'] = 'procurement/store_request_edit/$1';
$route['procurement/division-po-sr'] = 'procurement/division_po_sr';
$route['procurement/division-po-sr/print'] = 'procurement/division_po_sr_print';
$route['procurement/division-po-sr/pdf'] = 'procurement/division_po_sr_pdf';
$route['procurement/division-po-sr/create'] = 'procurement/division_po_sr_create';
$route['procurement/division-po-sr/edit/(:num)'] = 'procurement/division_po_sr_edit/$1';
$route['procurement/division-po-sr/profile-search'] = 'procurement/division_po_sr_profile_search';
$route['procurement/division-po-sr/stock-preview'] = 'procurement/division_stock_preview';
$route['procurement/division-po-sr/store'] = 'procurement/division_po_sr_store';
$route['procurement/division-po-sr/detail/(:num)'] = 'procurement/division_po_sr_detail/$1';
$route['procurement/division-po-sr/notify/(:num)'] = 'procurement/division_po_sr_notify/$1';
$route['wa/notification-settings'] = 'whatsapp/notification_settings';
$route['telegram/notification-settings'] = 'telegram/notification_settings';
$route['procurement/division-po-sr/verify/(:num)'] = 'procurement/division_po_sr_verify/$1';
$route['procurement/division-po-sr/action/(:num)'] = 'procurement/division_po_sr_action/$1';
$route['procurement/store-request/profile-search'] = 'procurement/store_request_profile_search';
$route['procurement/store-request/stock-preview'] = 'procurement/store_request_stock_preview';
$route['purchase-orders/stock-preview'] = 'purchase/order_stock_preview';
$route['procurement/store-request/store'] = 'procurement/store_request_store';
$route['procurement/store-request/update/(:num)'] = 'procurement/store_request_update/$1';
$route['procurement/store-request/action/(:num)'] = 'procurement/store_request_action/$1';
$route['procurement/store-request/split-preview/(:num)'] = 'procurement/store_request_split_preview/$1';
$route['procurement/store-request/fulfill/(:num)'] = 'procurement/store_request_fulfill/$1';
$route['procurement/store-request/repair-history/(:num)'] = 'procurement/store_request_repair_history/$1';
$route['procurement/store-request/generate-po/(:num)'] = 'procurement/store_request_generate_po/$1';

$route['purchase-orders'] = 'purchase/index';
$route['purchase-orders/create'] = 'purchase/order_create';
$route['purchase-orders/edit/(:num)'] = 'purchase/order_edit/$1';
$route['purchase-orders/detail/(:num)'] = 'purchase/order_detail/$1';
$route['purchase-orders/logs'] = 'purchase/order_log_index';
$route['purchase-orders/receipt'] = 'purchase/receipt_index';
$route['purchase-orders/report'] = 'purchase/report_index';
$route['purchase-orders/report/detail'] = 'purchase/report_detail_index';
$route['purchase-orders/report/detail-data'] = 'purchase/report_detail_data';
$route['purchase'] = 'purchase/index';
$route['purchase/account'] = 'purchase/account_index';
$route['purchase/stock/warehouse'] = 'inventory_warehouse/index';
$route['purchase/stock/division'] = 'inventory_division/index';
$route['purchase/receipt'] = 'purchase/receipt_index';
$route['purchase/receipt/po-lines'] = 'purchase/receipt_po_lines';
$route['purchase/receipt/store'] = 'purchase/receipt_store';
$route['purchase/catalog/search'] = 'purchase/catalog_search';
$route['purchase/catalog/sync-core'] = 'purchase/catalog_sync_core';
$route['purchase/vendor/quick-store'] = 'purchase/vendor_quick_store';
$route['purchase/order/store'] = 'purchase/order_store';
$route['purchase/order/update/(:num)'] = 'purchase/order_update/$1';
$route['purchase/order/status-update'] = 'purchase/order_status_update';
$route['purchase/order/logs'] = 'purchase/order_log_index';
$route['purchase/report'] = 'purchase/report_index';
$route['purchase/report/detail'] = 'purchase/report_detail_index';
$route['purchase/report/detail-data'] = 'purchase/report_detail_data';
$route['purchase/rebuild-impact'] = 'purchase/rebuild_impact_index';
$route['purchase/rebuild-impact/run'] = 'purchase/rebuild_impact_run';
$route['purchase/reclassify-profile-domain'] = 'purchase/reclassify_profile_domain_index';
$route['purchase/reclassify-profile-domain/run'] = 'purchase/reclassify_profile_domain_run';
$route['purchase/payment/apply'] = 'purchase/payment_apply';
$route['finance/accounts'] = 'master/index/company-account';
$route['finance/mutations'] = 'purchase/finance_mutation_index';
$route['finance/mutations/store'] = 'purchase/finance_mutation_store';
$route['finance/mutations/classify'] = 'purchase/finance_mutation_classify';
$route['finance/utang'] = 'finance/utang';
$route['finance/utang/store'] = 'finance/utang_store';
$route['finance/utang/update/(:num)'] = 'finance/utang_update/$1';
$route['finance/utang/payment/(:num)'] = 'finance/utang_payment/$1';
$route['finance/utang/void/(:num)'] = 'finance/utang_void/$1';
$route['finance/utang/delete/(:num)'] = 'finance/utang_delete/$1';
$route['finance/piutang'] = 'finance/piutang';
$route['finance/piutang/store'] = 'finance/piutang_store';
$route['finance/piutang/update/(:num)'] = 'finance/piutang_update/$1';
$route['finance/piutang/payment/(:num)'] = 'finance/piutang_payment/$1';
$route['finance/piutang/void/(:num)'] = 'finance/piutang_void/$1';
$route['finance/piutang/delete/(:num)'] = 'finance/piutang_delete/$1';
$route['finance/relasi'] = 'finance/parties';
$route['finance/relasi/save'] = 'finance/party_save';
$route['finance/relasi/toggle/(:num)'] = 'finance/party_toggle/$1';
$route['finance/relasi/delete/(:num)'] = 'finance/party_delete/$1';
$route['finance/party-search'] = 'finance/party_search';
$route['finance/member-search'] = 'finance/member_search';
$route['finance-reports/financial-estimation'] = 'finance_reports/financial_estimation';
$route['finance-reports/daily-overview'] = 'finance_reports/daily_overview';
$route['finance-reports/rekap-rekening-harian'] = 'finance_reports/bank_daily_recap';
$route['finance-reports/cash-position'] = 'finance_reports/cash_position';
$route['finance-reports/cash-vault-daily'] = 'finance_reports/cash_vault_daily';
$route['finance-reports/cash-reconciliation'] = 'finance_reports/cash_reconciliation';
$route['finance-reports/cash-reconciliation/round-create'] = 'finance_reports/cash_reconciliation_round_create';
$route['finance-reports/cash-reconciliation/line-save'] = 'finance_reports/cash_reconciliation_line_save';
$route['finance-reports/cash-reconciliation/line-post'] = 'finance_reports/cash_reconciliation_line_post';
$route['finance-reports/revenue-reconciliation'] = 'finance_reports/revenue_reconciliation';
$route['finance-reports/revenue-reconciliation/round-create'] = 'finance_reports/revenue_reconciliation_round_create';
$route['finance-reports/revenue-reconciliation/line-save'] = 'finance_reports/revenue_reconciliation_line_save';
$route['finance-reports/revenue-reconciliation/line-post'] = 'finance_reports/revenue_reconciliation_line_post';
$route['finance-reports/period-close'] = 'finance_reports/period_close';
$route['finance-reports/period-close/detail/(:num)'] = 'finance_reports/period_close_detail/$1';
$route['finance-reports/period-close/store'] = 'finance_reports/period_close_store';
$route['finance-reports/period-close/process/(:num)'] = 'finance_reports/period_close_process/$1';
$route['finance-reports/period-close/reopen/(:num)'] = 'finance_reports/period_close_reopen/$1';
$route['finance-reports/targets'] = 'finance_reports/targets';
$route['finance-reports/targets/detail/(:num)'] = 'finance_reports/target_detail/$1';
$route['finance-reports/targets/store'] = 'finance_reports/target_store';
$route['finance-reports/targets/generate-daily-range'] = 'finance_reports/target_generate_daily_range';
$route['finance-reports/targets/realize-bulk'] = 'finance_reports/target_realize_bulk';
$route['finance-reports/targets/realize/(:num)'] = 'finance_reports/target_realize/$1';
$route['finance-reports/targets/update/(:num)'] = 'finance_reports/target_update/$1';
$route['finance-reports/targets/lines-save/(:num)'] = 'finance_reports/target_lines_save/$1';
$route['purchase/stock/opening'] = 'inventory/stock_opening_index';
$route['purchase/stock/opening/warehouse'] = 'inventory_warehouse/opening';
$route['purchase/stock/stok-awal/warehouse'] = 'inventory_warehouse/stok_awal';
$route['purchase/stock/opening/division'] = 'inventory_division/opening';
$route['purchase/stock/division/transfer'] = 'inventory_division/transfer';
$route['purchase/stock/opening/item-search'] = 'inventory/stock_opening_item_search';
$route['purchase/stock/opening/store'] = 'inventory/stock_opening_store';
$route['purchase/stock/opname/generate'] = 'inventory/stock_opname_generate';
$route['purchase/stock/warehouse/daily'] = 'inventory_warehouse/daily';
$route['purchase/stock/warehouse/daily-matrix'] = 'inventory_warehouse/daily_matrix';
$route['purchase/stock/warehouse/daily-matrix-view'] = 'inventory_warehouse/matrix_view';
$route['purchase/stock/warehouse/movement'] = 'inventory_warehouse/movement';
$route['purchase/stock/division/movement'] = 'inventory_division/movement';
$route['purchase/stock/division/daily'] = 'inventory_division/daily';
$route['purchase/stock/division/reconcile'] = 'inventory_division/compare';
$route['purchase/stock/division/reconcile/audit'] = 'inventory_division/reconcile_audit';
$route['purchase/stock/division/reconcile/repair'] = 'inventory_division/reconcile_repair';
$route['purchase/stock/division/reconcile/lot-repair'] = 'inventory_division/reconcile_lot_repair';
$route['purchase/stock/division/reconcile/lot-profile-sync'] = 'inventory_division/reconcile_lot_profile_sync';
$route['purchase/stock/division/reconcile/lot-repair-all']  = 'inventory_division/reconcile_lot_repair_all';
$route['purchase/stock/division/reconcile/lot-only-adjust'] = 'inventory_division/reconcile_lot_only_adjust';
$route['purchase/stock/division/reconcile/repair-material-id'] = 'inventory_division/reconcile_repair_material_id';
$route['purchase/stock/division/reconcile/profile-repair']     = 'inventory_division/reconcile_profile_repair';
$route['purchase/stock/division/reconcile/profile-merge']      = 'inventory_division/reconcile_profile_merge';
$route['purchase/stock/material/daily-matrix'] = 'inventory_division/material_matrix';
$route['purchase/stock/material/daily-matrix-view'] = 'inventory_division/matrix_view';
$route['inventory/stock/warehouse'] = 'inventory_warehouse/index';
$route['inventory/stock/division'] = 'inventory_division/index';
$route['inventory/stock/opening'] = 'inventory/stock_opening_index';
$route['inventory/stock/opening/warehouse'] = 'inventory_warehouse/opening';
$route['inventory/stock/stok-awal/warehouse'] = 'inventory_warehouse/stok_awal';
$route['inventory/stock/opening/division'] = 'inventory_division/opening';
$route['inventory/stock/stok-awal/division'] = 'inventory_division/stok_awal';
$route['inventory/stock/adjustment'] = 'inventory/stock_adjustment_index';
$route['inventory/stock/adjustment/warehouse'] = 'inventory_warehouse/adjustment';
$route['inventory/stock/adjustment/division'] = 'inventory_division/adjustment';
$route['inventory/stock/transfer/division'] = 'inventory_division/transfer';
$route['inventory/stock/daily-recon/division']               = 'inventory_division/opname';
$route['inventory/stock/daily-recon/division/data']          = 'inventory_division/opname_data';
$route['inventory/stock/daily-recon/division/save-physical'] = 'inventory_division/opname_save_physical';
$route['inventory/stock/daily-recon/division/quick-adjust']  = 'inventory_division/opname_quick_adjust';
$route['inventory/stock/daily-recon/division/confirm']       = 'inventory_division/opname_confirm_recon';
$route['inventory/stock/daily-recon/warehouse'] = 'inventory_warehouse/recon';
$route['inventory/stock/daily-recon/warehouse/quick-adjust'] = 'inventory_warehouse/recon_quick_adjust';
$route['inventory/stock/koreksi/division']               = 'inventory_division/opname';
$route['inventory/stock/koreksi/division/data']          = 'inventory_division/opname_data';
$route['inventory/stock/koreksi/division/save-physical'] = 'inventory_division/opname_save_physical';
$route['inventory/stock/koreksi/division/quick-adjust']  = 'inventory_division/opname_quick_adjust';
$route['inventory/stock/opname/division']              = 'inventory_division/opname';
$route['inventory/stock/opname/division/data']         = 'inventory_division/opname_data';
$route['inventory/stock/opname/division/save-physical']= 'inventory_division/opname_save_physical';
$route['inventory/stock/opname/division/quick-adjust'] = 'inventory_division/opname_quick_adjust';
$route['inventory/stock/warehouse/lot'] = 'inventory_warehouse/lot';
$route['inventory/stock/division/lot'] = 'inventory_division/lot';
$route['inventory/stock/opening/item-search'] = 'inventory/stock_opening_item_search';
$route['inventory/stock/opening/store'] = 'inventory/stock_opening_store';
$route['inventory/stock/opening/step-up/verify'] = 'inventory/stock_opening_step_up_verify';
$route['inventory/stock/opening/division/export-template'] = 'purchase/stock_opening_division_export_template';
$route['inventory/stock/opening/division/export-existing'] = 'purchase/stock_opening_division_export_existing';
$route['inventory/stock/opening/division/import'] = 'purchase/stock_opening_division_import';
$route['inventory/stock/opening/void/(:num)'] = 'inventory/stock_opening_void/$1';
$route['inventory/stock/adjustment/item-search'] = 'inventory/stock_adjustment_item_search';
$route['inventory/stock/adjustment/store'] = 'inventory/stock_adjustment_store';
$route['inventory/stock/adjustment/step-up/verify'] = 'inventory/stock_adjustment_step_up_verify';
$route['inventory/stock/adjustment/post/(:num)'] = 'inventory/stock_adjustment_post/$1';
$route['inventory/stock/adjustment/void/(:num)'] = 'inventory/stock_adjustment_void/$1';
$route['inventory/stock/adjustment/delete/(:num)'] = 'inventory/stock_adjustment_delete/$1';
$route['inventory/stock/deficits'] = 'inventory_control/deficits';
$route['inventory/stock/deficits/detail/(:num)'] = 'inventory_control/deficit_detail/$1';
$route['inventory/stock/deficits/write-off/(:num)'] = 'inventory_control/deficit_write_off/$1';
$route['inventory/stock/health'] = 'inventory_control/health';
$route['inventory/stock/integrity-audit'] = 'inventory_control/integrity_audit';
$route['inventory/stock/value-reconciliation'] = 'inventory_control/value_reconciliation';
$route['inventory/stock/value-reconciliation/post'] = 'inventory_control/value_reconciliation_post';
$route['inventory/stock/value-reconciliation/void'] = 'inventory_control/value_reconciliation_void';
$route['inventory/stock/periods'] = 'inventory_control/periods';
$route['inventory/stock/periods/detail/(:num)'] = 'inventory_control/period_detail/$1';
$route['inventory/stock/periods/open'] = 'inventory_control/period_open';
$route['inventory/stock/periods/cutoff-post/(:num)'] = 'inventory_control/period_cutoff_post/$1';
$route['inventory/stock/periods/close/(:num)'] = 'inventory_control/period_close/$1';
$route['inventory/stock/periods/reopen/(:num)'] = 'inventory_control/period_reopen/$1';
$route['inventory/stock/transfer/item-search'] = 'inventory/stock_transfer_item_search';
$route['inventory/stock/transfer/store'] = 'inventory/stock_transfer_store';
$route['inventory/stock/transfer/step-up/verify'] = 'inventory/stock_transfer_step_up_verify';
$route['inventory/stock/transfer/post/(:num)'] = 'inventory/stock_transfer_post/$1';
$route['inventory/stock/transfer/void/(:num)'] = 'inventory/stock_transfer_void/$1';
$route['inventory/stock/transfer/delete/(:num)'] = 'inventory/stock_transfer_delete/$1';
$route['inventory/stock/opname/generate']             = 'inventory/stock_opname_generate';
$route['inventory/stock/opname/warehouse/monthly']    = 'inventory_warehouse/opname_monthly';
$route['inventory/stock/opname/division/monthly']     = 'inventory_division/opname_monthly';
$route['inventory/stock/warehouse/daily'] = 'inventory_warehouse/daily';
$route['inventory/stock/warehouse/movement'] = 'inventory_warehouse/movement';
$route['inventory/stock/warehouse/daily-matrix'] = 'inventory_warehouse/daily_matrix';
$route['inventory/stock/division/movement'] = 'inventory_division/movement';
$route['inventory/stock/division/daily'] = 'inventory_division/daily';
$route['inventory/stock/division/reconcile'] = 'inventory_division/compare';
$route['inventory/stock/division/reconcile/audit'] = 'inventory_division/reconcile_audit';
$route['inventory/stock/division/reconcile/repair'] = 'inventory_division/reconcile_repair';
$route['inventory/stock/division/reconcile/lot-repair'] = 'inventory_division/reconcile_lot_repair';
$route['inventory/stock/division/reconcile/lot-profile-sync'] = 'inventory_division/reconcile_lot_profile_sync';
$route['inventory/stock/division/reconcile/lot-repair-all']  = 'inventory_division/reconcile_lot_repair_all';
$route['inventory/stock/division/reconcile/lot-only-adjust'] = 'inventory_division/reconcile_lot_only_adjust';
$route['inventory/stock/division/reconcile/gap-repair-all']  = 'inventory_division/reconcile_gap_repair_all';
$route['inventory/stock/division/reconcile/log-repair']      = 'inventory_division/reconcile_log_repair';
$route['inventory/stock/division/reconcile/repair-material-id'] = 'inventory_division/reconcile_repair_material_id';
$route['inventory/stock/division/reconcile/profile-repair']     = 'inventory_division/reconcile_profile_repair';
$route['inventory/stock/division/reconcile/profile-merge']      = 'inventory_division/reconcile_profile_merge';
$route['inventory/stock/material/daily-matrix'] = 'inventory_division/material_matrix';
$route['inventory-warehouse-daily/matrix'] = 'inventory_warehouse/daily_matrix';
$route['inventory-material-daily/matrix'] = 'inventory_division/material_matrix';
$route['inventory-warehouse-daily'] = 'inventory_warehouse/matrix_view';
$route['inventory-material-daily'] = 'inventory_division/matrix_view';
$route['inventory-daily/cell-detail'] = 'inventory/stock_daily_cell_detail';
$route['inventory/fifo-audit'] = 'inventory/fifo_audit';
$route['inventory/lot-audit'] = 'inventory/lot_audit';
$route['purchase/setup/sync-core'] = 'purchase/setup_sync_core';
$route['purchase/setup/sync-core-all'] = 'purchase/setup_sync_core_all';

// Production - Base/Prepare (read-only stage)
$route['production/component-stock'] = 'production/component_stock';
$route['production/component-stock/data'] = 'production/component_stock_data';
$route['production/component-movements'] = 'production/component_movements';
$route['production/component-movements/data'] = 'production/component_movements_data';
$route['production/component-daily'] = 'production/component_daily';
$route['production/component-daily/data'] = 'production/component_daily_data';
$route['production/component-monthly'] = 'production/component_monthly';
$route['production/component-reconcile'] = 'production/component_reconcile';
$route['production/component-reconcile/audit'] = 'production/component_reconcile_audit';
$route['production/component-reconcile/repair']     = 'production/component_reconcile_repair';
$route['production/component-reconcile/repair-all'] = 'production/component_reconcile_repair_all';
$route['production/component-reconcile/lot-repair']    = 'production/component_lot_repair';
$route['production/component-reconcile/lot-adjust']    = 'production/component_lot_only_adjust';
$route['production/component-reconcile/lot-sync']      = 'production/component_lot_sync_to_stock';
$route['production/component-reconcile/lot-sync-all']  = 'production/component_lot_sync_all';
$route['production/component-reconcile/fix-log']       = 'production/component_movement_log_fix_to_stock';
$route['production/component-lots'] = 'production/component_lots';
$route['production/component-openings'] = 'production/component_openings';
$route['production/component-openings/export-template'] = 'production/component_opening_export_template';
$route['production/component-openings/export-existing'] = 'production/component_opening_export_existing';
$route['production/component-openings/import'] = 'production/component_opening_import';
$route['production/component-openings/detail/(:num)'] = 'production/component_opening_detail/$1';
$route['production/component-openings/save'] = 'production/component_opening_save';
$route['production/component-openings/post/(:num)'] = 'production/component_opening_post/$1';
$route['production/component-openings/delete/(:num)'] = 'production/component_opening_delete/$1';
$route['production/component-openings/reopen/(:num)'] = 'production/component_opening_reopen/$1';
$route['production/component-openings/void/(:num)'] = 'production/component_opening_void/$1';
$route['production/component-openings/generate-monthly'] = 'production/component_opening_generate_monthly';
$route['production/component-opening-monthly']           = 'production/component_opening_monthly';
$route['production/component-opname']                    = 'production/component_opname';
$route['production/component-adjustments'] = 'production/component_adjustments';
$route['production/component-adjustments/save'] = 'production/component_adjustment_save';
$route['production/component-adjustments/step-up/verify'] = 'production/component_adjustment_step_up_verify';
$route['production/component-adjustments/void-step-up/verify'] = 'production/component_adjustment_void_step_up_verify';
$route['production/component-adjustments/post/(:num)'] = 'production/component_adjustment_post/$1';
$route['production/component-adjustments/void/(:num)'] = 'production/component_adjustment_void/$1';
$route['production/component-adjustments/delete/(:num)'] = 'production/component_adjustment_delete/$1';
$route['production/component-stock-snapshot'] = 'production/component_stock_snapshot';
$route['production/component-batches'] = 'production/component_batches';
$route['production/component-batches/save'] = 'production/component_batch_save';
$route['production/component-batches/preview'] = 'production/component_batch_preview';
$route['production/component-batches/step-up/verify'] = 'production/component_batch_step_up_verify';
$route['production/component-batches/void-step-up/verify'] = 'production/component_batch_void_step_up_verify';
$route['production/component-batches/post/(:num)'] = 'production/component_batch_post/$1';
$route['production/component-batches/status/(:num)'] = 'production/component_batch_status/$1';
$route['production/component-batches/usage/(:num)'] = 'production/component_batch_usage/$1';
$route['production/component-batches/detail/(:num)'] = 'production/component_batch_usage_page/$1';
$route['production/component-daily-recon']               = 'production/component_daily_recon';
$route['production/component-daily-recon/data']          = 'production/component_daily_recon_data';
$route['production/component-daily-recon/save-physical'] = 'production/component_daily_recon_save';
$route['production/component-daily-recon/step-up/verify'] = 'production/component_daily_recon_step_up_verify';
$route['production/component-daily-recon/quick-adjust']  = 'production/component_daily_recon_adjust';
$route['production/component-daily-recon/confirm']       = 'production/component_daily_recon_confirm';
$route['production/component-batches/void/(:num)'] = 'production/component_batch_void/$1';
$route['production/component-batches/delete/(:num)'] = 'production/component_batch_delete/$1';
$route['production/component-picker-search'] = 'production/component_picker_search';
$route['production/component-categories'] = 'production/component_categories';
$route['production/component-categories/save'] = 'production/component_category_save';
$route['production/component-categories/toggle/(:num)'] = 'production/component_category_toggle/$1';
$route['production/component-categories/quick-map'] = 'production/component_category_quick_map';
$route['production/component-masters'] = 'production/component_masters';
$route['production/component-masters/data'] = 'production/component_masters_data';
$route['production/component-masters/save'] = 'production/component_master_save';
$route['production/component-masters/toggle/(:num)'] = 'production/component_master_toggle/$1';
$route['production/component-masters/usage/(:num)'] = 'production/component_master_usage/$1';
$route['production/component-formulas'] = 'production/component_formulas';
$route['production/component-formulas/data'] = 'production/component_formulas_data';
$route['production/component-formulas/detail-data'] = 'production/component_formula_detail';
$route['production/component-formulas/source-search'] = 'production/component_formula_source_search';
$route['production/component-formulas/detail/(:num)'] = 'production/component_formula_show/$1';
$route['production/component-formulas/edit/(:num)'] = 'production/component_formula_edit/$1';
$route['production/component-formulas/save'] = 'production/component_formula_save';
$route['production/component-formulas/save-bulk'] = 'production/component_formula_save_bulk';
$route['production/component-formulas/delete/(:num)'] = 'production/component_formula_delete/$1';
$route['production/component-formulas/restore-step-up/verify'] = 'production/component_formula_restore_step_up_verify';
$route['production/component-formulas/restore'] = 'production/component_formula_restore';
$route['production/component-cost-variables'] = 'production/component_cost_variables';
$route['production/component-cost-variables/save'] = 'production/component_cost_variable_save';

// Roastery
$route['roastery/packaging-labels'] = 'roastery/packaging_labels';
$route['roastery/packaging-labels/save'] = 'roastery/packaging_label_save';
$route['roastery/packaging-labels/templates/save'] = 'roastery/packaging_label_template_save';
$route['roastery/packaging-labels/templates/delete/(:num)'] = 'roastery/packaging_label_template_delete/$1';
$route['roastery/packaging-labels/print/(:num)'] = 'roastery/packaging_label_print/$1';
$route['roastery/packaging-labels/duplicate/(:num)'] = 'roastery/packaging_label_duplicate/$1';
$route['roastery/packaging-labels/activate/(:num)'] = 'roastery/packaging_label_activate/$1';
$route['roastery/packaging-labels/delete/(:num)'] = 'roastery/packaging_label_delete/$1';

// Asset Management
$route['asset-management'] = 'assets/index';
$route['asset-management/create'] = 'assets/create';
$route['asset-management/store'] = 'assets/store';
$route['asset-management/lock-bulk'] = 'assets/lock_bulk';
$route['asset-management/lock/(:num)'] = 'assets/lock_asset/$1';
$route['asset-management/changes'] = 'assets/changes';
$route['asset-management/changes/create/(:num)'] = 'assets/change_create/$1';
$route['asset-management/changes/store/(:num)'] = 'assets/change_store/$1';
$route['asset-management/changes/(:num)/approve'] = 'assets/change_approve/$1';
$route['asset-management/changes/(:num)/reject'] = 'assets/change_reject/$1';
$route['asset-management/changes/(:num)/post'] = 'assets/change_post/$1';
$route['asset-management/changes/(:num)/cancel'] = 'assets/change_cancel/$1';
$route['asset-management/changes/(:num)'] = 'assets/change_detail/$1';
$route['asset-management/group/(:any)/quantity'] = 'assets/group_quantity/$1';
$route['asset-management/group/(:any)'] = 'assets/group/$1';
$route['asset-management/detail/(:num)'] = 'assets/detail/$1';
$route['asset-management/edit/(:num)'] = 'assets/edit/$1';
$route['asset-management/update/(:num)'] = 'assets/update/$1';
$route['asset-management/delete/(:num)'] = 'assets/delete/$1';
$route['asset-management/search-assets'] = 'assets/asset_search';
$route['asset-management/damage'] = 'assets/damage_index';
$route['asset-management/damage/create'] = 'assets/damage_create';
$route['asset-management/damage/store'] = 'assets/damage_store';
$route['asset-management/damage/asset-search'] = 'assets/damage_asset_search';
$route['asset-management/damage/edit/(:num)'] = 'assets/damage_edit/$1';
$route['asset-management/damage/update/(:num)'] = 'assets/damage_update/$1';
$route['asset-management/damage/delete/(:num)'] = 'assets/damage_delete/$1';
$route['asset-management/damage/(:num)'] = 'assets/damage/$1';
$route['asset-management/damage/(:num)/store'] = 'assets/damage_store/$1';
$route['asset-management/recon'] = 'assets/recon';
$route['asset-management/recon/generate'] = 'assets/recon_generate';
$route['asset-management/recon/(:num)'] = 'assets/recon_detail/$1';
$route['asset-management/recon/(:num)/save'] = 'assets/recon_save/$1';
$route['asset-management/recon/(:num)/post'] = 'assets/recon_post/$1';
$route['asset-management/recon/(:num)/cancel'] = 'assets/recon_cancel/$1';
$route['asset-management/labels'] = 'assets/labels';
$route['asset-management/transfer'] = 'assets/transfer';
$route['asset-management/transfer/create'] = 'assets/transfer_create';
$route['asset-management/transfer/store'] = 'assets/transfer_store';
$route['asset-management/transfer/(:num)/approve'] = 'assets/transfer_approve/$1';
$route['asset-management/transfer/(:num)/reject'] = 'assets/transfer_reject/$1';
$route['asset-management/transfer/(:num)/cancel'] = 'assets/transfer_cancel/$1';
$route['asset-management/transfer/(:num)/post'] = 'assets/transfer_post/$1';
$route['asset-management/handover'] = 'assets/handover';
$route['asset-management/handover/create'] = 'assets/handover_create';
$route['asset-management/handover/store'] = 'assets/handover_store';
$route['asset-management/handover/(:num)/approve'] = 'assets/handover_approve/$1';
$route['asset-management/handover/(:num)/reject'] = 'assets/handover_reject/$1';
$route['asset-management/handover/(:num)/cancel'] = 'assets/handover_cancel/$1';
$route['asset-management/handover/(:num)/post'] = 'assets/handover_post/$1';
$route['asset-management/maintenance'] = 'assets/maintenance';
$route['asset-management/maintenance/create'] = 'assets/maintenance_create';
$route['asset-management/maintenance/store'] = 'assets/maintenance_store';
$route['asset-management/maintenance/(:num)/approve'] = 'assets/maintenance_approve/$1';
$route['asset-management/maintenance/(:num)/reject'] = 'assets/maintenance_reject/$1';
$route['asset-management/maintenance/(:num)/cancel'] = 'assets/maintenance_cancel/$1';
$route['asset-management/maintenance/(:num)/complete'] = 'assets/maintenance_complete/$1';
$route['asset-management/disposal'] = 'assets/disposal';
$route['asset-management/disposal/create'] = 'assets/disposal_create';
$route['asset-management/disposal/store'] = 'assets/disposal_store';
$route['asset-management/disposal/(:num)/approve'] = 'assets/disposal_approve/$1';
$route['asset-management/disposal/(:num)/reject'] = 'assets/disposal_reject/$1';
$route['asset-management/disposal/(:num)/cancel'] = 'assets/disposal_cancel/$1';
$route['asset-management/disposal/(:num)/post'] = 'assets/disposal_post/$1';
$route['asset-management/depreciation'] = 'assets/depreciation';
$route['asset-management/depreciation/generate'] = 'assets/depreciation_generate';
$route['asset-management/depreciation/(:num)/post'] = 'assets/depreciation_post/$1';
$route['asset-management/depreciation/(:num)/cancel'] = 'assets/depreciation_cancel/$1';

// POS Master
$route['master/lookup-search/(:any)/(:any)'] = 'master/lookup_search/$1/$2';
$route['pos/members'] = 'pos/members';
$route['pos/members/data'] = 'pos/members_data';
$route['pos/members/save'] = 'pos/member_save';
$route['pos/members/toggle/(:num)'] = 'pos/member_toggle/$1';
$route['pos/payment-methods'] = 'pos/payment_methods';
$route['pos/payment-methods/data'] = 'pos/payment_methods_data';
$route['pos/payment-methods/save'] = 'pos/payment_method_save';
$route['pos/payment-methods/toggle/(:num)'] = 'pos/payment_method_toggle/$1';
$route['pos/deposits'] = 'pos/deposits';
$route['pos/deposits/data'] = 'pos/deposits_data';
$route['pos/stock-commit-audit'] = 'pos/stock_commit_audit';
$route['pos/stock-commit-audit/repair-material-mismatches'] = 'pos/stock_commit_audit_repair_material_mismatches';
$route['pos/stock-commit-audit/repair-component-mismatches'] = 'pos/stock_commit_audit_repair_component_mismatches';
$route['pos/stock-commit-audit/repair-material-drift'] = 'pos/stock_commit_audit_repair_material_drift';
$route['pos/stock-commit-audit/repair-component-drift'] = 'pos/stock_commit_audit_repair_component_drift';
$route['pos/deposits/member-search'] = 'pos/deposit_member_search';
$route['pos/deposits/save'] = 'pos/deposit_save';
$route['pos/deposits/void/(:num)'] = 'pos/deposit_void/$1';
$route['pos/self-order'] = 'pos/self_order';
$route['pos/self-order/settings'] = 'pos/self_order_settings';
$route['pos/self-order/tables'] = 'pos/self_order_tables';
$route['pos/self-order/tables/data'] = 'pos/self_order_tables_data';
$route['pos/self-order/tables/save']      = 'pos/self_order_table_save';
$route['pos/self-order/tables/bulk-save'] = 'pos/self_order_table_bulk_save';
$route['pos/self-order/tables/delete/(:num)'] = 'pos/self_order_table_delete/$1';
$route['pos/self-order/tables/print'] = 'pos/self_order_tables_print';
$route['pos/self-order/orders'] = 'pos/self_order_orders';
$route['pos/self-order/orders/data'] = 'pos/self_order_orders_data';
$route['pos/self-order/orders/detail/(:num)'] = 'pos/self_order_order_detail/$1';
$route['pos/self-order/orders/verify/(:num)'] = 'pos/self_order_order_verify/$1';
$route['pos/self-order/orders/reject/(:num)'] = 'pos/self_order_order_reject/$1';
$route['pos/reservations'] = 'pos/reservations';
$route['pos/reservations/data'] = 'pos/reservations_data';
$route['pos/reservations/products/data'] = 'pos/reservation_products_data';
$route['pos/reservations/detail/(:num)'] = 'pos/reservation_detail/$1';
$route['pos/reservations/catalog'] = 'pos/reservation_catalog';
$route['pos/reservations/bundles'] = 'pos/reservation_bundle_catalog';
$route['pos/reservations/extras'] = 'pos/reservation_extra_options';
$route['pos/reservations/members'] = 'pos/reservation_member_search';
$route['pos/reservations/save'] = 'pos/reservation_save';
$route['pos/reservations/deposit/(:num)'] = 'pos/reservation_deposit/$1';
$route['pos/reservations/verify/(:num)'] = 'pos/reservation_verify/$1';
$route['pos/reservations/refund-step-up/verify'] = 'pos/reservation_refund_step_up_verify';
$route['pos/reservations/reject/(:num)'] = 'pos/reservation_reject/$1';
$route['pos/reservations/cancel/(:num)'] = 'pos/reservation_cancel/$1';
$route['pos/online-food'] = 'pos/online_food';
$route['pos/online-food/orders'] = 'pos/online_food_orders';
$route['pos/online-food/orders/data'] = 'pos/online_food_orders_data';
$route['pos/online-food/orders/detail/(:num)'] = 'pos/online_food_order_detail/$1';
$route['pos/online-food/orders/verify/(:num)'] = 'pos/online_food_order_verify/$1';
$route['pos/online-food/orders/reject/(:num)'] = 'pos/online_food_order_reject/$1';
$route['pos/online-food/settings'] = 'pos/online_food_settings';
$route['pos/online-food/locations'] = 'pos/online_food_locations';
$route['pos/online-food/locations/data'] = 'pos/online_food_locations_data';
$route['pos/online-food/locations/member-search'] = 'pos/online_food_location_member_search';
$route['pos/online-food/locations/save'] = 'pos/online_food_location_save';
$route['pos/online-food/locations/delete/(:num)'] = 'pos/online_food_location_delete/$1';
$route['pos/outlets-terminals'] = 'pos/outlets_terminals';
$route['pos/outlets/data'] = 'pos/outlets_data';
$route['pos/outlets/save'] = 'pos/outlet_save';
$route['pos/outlets/toggle/(:num)'] = 'pos/outlet_toggle/$1';
$route['pos/terminals/data'] = 'pos/terminals_data';
$route['pos/terminals/save'] = 'pos/terminal_save';
$route['pos/terminals/toggle/(:num)'] = 'pos/terminal_toggle/$1';

// Loyalty
$route['loyalty/members'] = 'loyalty/members';
$route['loyalty/members/data'] = 'loyalty/members_data';
$route['loyalty/members/save'] = 'loyalty/member_save';
$route['loyalty/members/toggle/(:num)'] = 'loyalty/member_toggle/$1';
$route['loyalty/members/delete/(:num)'] = 'loyalty/member_delete/$1';
$route['loyalty/members/(:num)/redeem/rules']   = 'loyalty/member_redeem_rules/$1';
$route['loyalty/members/(:num)/redeem/process'] = 'loyalty/member_redeem_process/$1';
$route['loyalty/members/(:num)/redeem/history'] = 'loyalty/member_redeem_history/$1';
$route['loyalty/members/(:num)/orders']         = 'loyalty/member_orders/$1';
$route['loyalty/members/(:num)/points']         = 'loyalty/member_points/$1';
$route['loyalty/members/(:num)/stamps']         = 'loyalty/member_stamps/$1';
$route['loyalty/members/(:num)/vouchers']       = 'loyalty/member_vouchers/$1';
$route['loyalty/members/(:num)']                = 'loyalty/member_detail/$1';
$route['loyalty/point-rules'] = 'loyalty/point_rules';
$route['loyalty/point-rules/data'] = 'loyalty/point_rules_data';
$route['loyalty/point-rules/save'] = 'loyalty/point_rule_save';
$route['loyalty/point-rules/toggle/(:num)'] = 'loyalty/point_rule_toggle/$1';
$route['loyalty/point-rules/delete/(:num)'] = 'loyalty/point_rule_delete/$1';
$route['loyalty/stamp-campaigns'] = 'loyalty/stamp_campaigns';
$route['loyalty/stamp-campaigns/data'] = 'loyalty/stamp_campaigns_data';
$route['loyalty/stamp-campaigns/save'] = 'loyalty/stamp_campaign_save';
$route['loyalty/stamp-campaigns/toggle/(:num)'] = 'loyalty/stamp_campaign_toggle/$1';
$route['loyalty/stamp-campaigns/delete/(:num)'] = 'loyalty/stamp_campaign_delete/$1';
$route['loyalty/voucher-campaigns'] = 'loyalty/voucher_campaigns';
$route['loyalty/voucher-campaigns/data'] = 'loyalty/voucher_campaigns_data';
$route['loyalty/voucher-campaigns/save'] = 'loyalty/voucher_campaign_save';
$route['loyalty/voucher-campaigns/toggle/(:num)'] = 'loyalty/voucher_campaign_toggle/$1';
$route['loyalty/voucher-campaigns/delete/(:num)'] = 'loyalty/voucher_campaign_delete/$1';
$route['loyalty/vouchers'] = 'loyalty/vouchers';
$route['loyalty/vouchers/data'] = 'loyalty/vouchers_data';
$route['loyalty/vouchers/save'] = 'loyalty/voucher_save';
$route['loyalty/vouchers/toggle/(:num)'] = 'loyalty/voucher_toggle/$1';
$route['loyalty/vouchers/delete/(:num)'] = 'loyalty/voucher_delete/$1';
$route['loyalty/voucher-usages'] = 'loyalty/voucher_usages';
$route['loyalty/voucher-usages/data'] = 'loyalty/voucher_usages_data';
$route['loyalty/voucher-usages/detail/(:num)'] = 'loyalty/voucher_usage_detail/$1';
$route['loyalty/product-search'] = 'loyalty/product_search';
$route['loyalty/member-search'] = 'loyalty/member_search';
$route['loyalty/redeem'] = 'loyalty/redeem_index';
$route['loyalty/redeem/data'] = 'loyalty/redeem_data';
$route['loyalty/redeem/member-info'] = 'loyalty/redeem_member_info';
$route['loyalty/redeem/point'] = 'loyalty/redeem_point_process';
$route['loyalty/redeem/stamp'] = 'loyalty/redeem_stamp_process';
$route['loyalty/redeem/voucher'] = 'loyalty/redeem_voucher_process';
$route['loyalty/redeem-rules'] = 'loyalty/redeem_rules';
$route['loyalty/redeem-rules/data'] = 'loyalty/redeem_rules_data';
$route['loyalty/redeem-rules/save'] = 'loyalty/redeem_rule_save';
$route['loyalty/redeem-rules/toggle/(:num)'] = 'loyalty/redeem_rule_toggle/$1';
$route['loyalty/redeem-rules/delete/(:num)'] = 'loyalty/redeem_rule_delete/$1';
$route['pos/printers'] = 'pos/printers';
$route['pos/printers/connections'] = 'pos/printer_connections';
$route['pos/printers/connections/data'] = 'pos/printer_connections_data';
$route['pos/printers/connections/save'] = 'pos/printer_connection_save';
$route['pos/printers/connections/toggle/(:num)'] = 'pos/printer_connection_toggle/$1';
$route['pos/printers/connections/test/(:num)'] = 'pos/printer_connection_test/$1';
$route['pos/printers/general'] = 'pos/printer_general';
$route['pos/printers/layouts'] = 'pos/printer_layouts';
$route['pos/printers/layouts/create'] = 'pos/printer_layout_editor';
$route['pos/printers/layouts/edit/(:num)'] = 'pos/printer_layout_editor/$1';
$route['pos/printers/layouts/data'] = 'pos/printer_layouts_data';
$route['pos/printers/layouts/save'] = 'pos/printer_layout_save';
$route['pos/printers/layouts/preview'] = 'pos/printer_layout_preview';
$route['pos/printers/layouts/test/(:num)'] = 'pos/printer_layout_test/$1';
$route['pos/printers/layouts/toggle/(:num)'] = 'pos/printer_layout_toggle/$1';
$route['pos/printers/rules'] = 'pos/printer_rules';
$route['pos/printers/rules/data'] = 'pos/printer_rules_data';
$route['pos/printers/rules/save'] = 'pos/printer_rule_save';
$route['pos/printers/rules/toggle/(:num)'] = 'pos/printer_rule_toggle/$1';
$route['pos/printers/preview-live'] = 'pos/printer_preview_live';
$route['pos/printers/preview-live/data/(:num)'] = 'pos/printer_preview_live_data/$1';
$route['pos/printers/monitor'] = 'pos/printer_monitor';
$route['pos/printers/monitor/data'] = 'pos/printer_monitor_data';
$route['pos/printers/attempts/ack'] = 'pos/printer_attempt_ack';
$route['pos/printers/guide'] = 'pos/printer_guide_config';
$route['pos/customer-reviews'] = 'pos/customer_reviews';
$route['pos/customer-reviews/data'] = 'pos/customer_reviews_data';
$route['pos/customer-reviews/visibility/(:num)'] = 'pos/customer_review_visibility/$1';
$route['pos/customer-reviews/settings'] = 'pos/customer_review_settings';
$route['pos/customer-reviews/stations/save'] = 'pos/customer_review_station_save';
$route['pos/customer-reviews/stations/toggle/(:num)'] = 'pos/customer_review_station_toggle/$1';
$route['pos/customer-reviews/stations/print/(:num)'] = 'pos/customer_review_station_print/$1';
$route['review/station/(:any)/submit'] = 'customer_reviews/station_submit/$1';
$route['review/station/(:any)'] = 'customer_reviews/station/$1';
$route['review/(:any)/submit'] = 'customer_reviews/submit/$1';
$route['review/(:any)'] = 'customer_reviews/index/$1';
$route['pos/printers/templates'] = 'pos/printer_templates';
$route['pos/printers/profiles'] = 'pos/printer_profiles';
$route['pos/printers/devices'] = 'pos/printer_devices';
$route['pos/printers/settings'] = 'pos/printer_settings';
$route['pos/printers/preview/(:num)'] = 'pos/printer_preview/$1';
$route['pos/printers/test/(:num)'] = 'pos/printer_test/$1';
$route['pos/printers/download/(:any)'] = 'pos/printer_download/$1';
$route['pos/printers/bootstrap'] = 'pos_printer_agent/bootstrap';
$route['pos-printers/bootstrap'] = 'pos_printer_agent/bootstrap';
$route['pos/printers/templates/create'] = 'pos/printer_template_create';
$route['pos/printers/templates/edit/(:num)'] = 'pos/printer_template_edit/$1';
$route['pos/printers/templates/preview/(:num)'] = 'pos/printer_template_preview/$1';
$route['pos/printers/templates/live-preview'] = 'pos/printer_template_live_preview';
$route['pos/printers/templates/data'] = 'pos/printer_templates_data';
$route['pos/printers/templates/save'] = 'pos/printer_template_save';
$route['pos/printers/templates/toggle/(:num)'] = 'pos/printer_template_toggle/$1';
$route['pos/printers/profiles/data'] = 'pos/printer_profiles_data';
$route['pos/printers/profiles/save'] = 'pos/printer_profile_save';
$route['pos/printers/profiles/toggle/(:num)'] = 'pos/printer_profile_toggle/$1';
$route['pos/printers/devices/data'] = 'pos/printer_devices_data';
$route['pos/printers/devices/save'] = 'pos/printer_device_save';
$route['pos/printers/devices/toggle/(:num)'] = 'pos/printer_device_toggle/$1';
$route['pos/sales-channels'] = 'pos/sales_channels';
$route['pos/sales-channels/data'] = 'pos/sales_channels_data';
$route['pos/sales-channels/save'] = 'pos/sales_channel_save';
$route['pos/sales-channels/toggle/(:num)'] = 'pos/sales_channel_toggle/$1';
$route['pos/sales-channels/delete/(:num)'] = 'pos/sales_channel_delete/$1';
$route['pos/cashier'] = 'pos/cashier';
$route['pos/cashier/open'] = 'pos/cashier_open';
$route['pos/cashier/recon-status'] = 'pos/cashier_recon_status';
$route['pos/cashier/close-preview'] = 'pos/cashier_close_preview';
$route['pos/cashier/close'] = 'pos/cashier_close';
$route['pos/daily-recon-settings'] = 'pos/daily_recon_settings';
$route['pos/daily-recon-settings/save'] = 'pos/daily_recon_settings_save';
$route['pos/cashier/session-status'] = 'pos/cashier_session_status';
$route['pos/cashier/catalog'] = 'pos/cashier_catalog';
$route['pos/cashier/bundles'] = 'pos/cashier_bundle_catalog';
$route['pos-mobile/ping'] = 'pos_mobile/ping';
$route['pos-mobile/auth/login'] = 'pos_mobile/login';
$route['pos-mobile/auth/logout'] = 'pos_mobile/logout';
$route['pos-mobile/bootstrap'] = 'pos_mobile/bootstrap';
$route['pos-mobile/catalog'] = 'pos_mobile/catalog';
$route['pos-mobile/members/search'] = 'pos_mobile/member_search';
$route['pos-mobile/products/extra-options'] = 'pos_mobile/extra_options';
$route['pos-mobile/printers'] = 'pos_mobile/printers';
$route['pos-mobile/printers/test/(:num)'] = 'pos_mobile/printer_test/$1';
$route['pos-mobile/reservations'] = 'pos_mobile/reservations';
$route['pos-mobile/reservations/products'] = 'pos_mobile/reservation_products';
$route['pos-mobile/reservations/verify/(:num)'] = 'pos_mobile/reservation_verify/$1';
$route['pos-mobile/reservations/reject-step-up/verify'] = 'pos_mobile/reservation_reject_step_up_verify';
$route['pos-mobile/reservations/reject/(:num)'] = 'pos_mobile/reservation_reject/$1';
$route['pos-mobile/reservations/(:num)'] = 'pos_mobile/reservation_detail/$1';
$route['pos-mobile/incoming/self-order'] = 'pos_mobile/self_order_inbox';
$route['pos-mobile/incoming/self-order/verify/(:num)'] = 'pos_mobile/self_order_inbox_verify/$1';
$route['pos-mobile/incoming/self-order/reject/(:num)'] = 'pos_mobile/self_order_inbox_reject/$1';
$route['pos-mobile/incoming/self-order/(:num)'] = 'pos_mobile/self_order_inbox_detail/$1';
$route['pos-mobile/incoming/online-food'] = 'pos_mobile/online_food_inbox';
$route['pos-mobile/incoming/online-food/verify/(:num)'] = 'pos_mobile/online_food_inbox_verify/$1';
$route['pos-mobile/incoming/online-food/reject/(:num)'] = 'pos_mobile/online_food_inbox_reject/$1';
$route['pos-mobile/incoming/online-food/(:num)'] = 'pos_mobile/online_food_inbox_detail/$1';
$route['pos-mobile/cashier/session-status'] = 'pos_mobile/session_status';
$route['pos-mobile/cashier/open'] = 'pos_mobile/cashier_open';
$route['pos-mobile/cashier/close-preview'] = 'pos_mobile/cashier_close_preview';
$route['pos-mobile/cashier/close-step-up/verify'] = 'pos_mobile/cashier_close_step_up_verify';
$route['pos-mobile/cashier/close'] = 'pos_mobile/cashier_close';
$route['pos-mobile/orders'] = 'pos_mobile/orders';
$route['pos-mobile/orders/(:num)'] = 'pos_mobile/order_load/$1';
$route['pos-mobile/orders/reversal-preview/(:num)'] = 'pos_mobile/order_reversal_preview/$1';
$route['pos-mobile/orders/reversal-step-up/verify'] = 'pos_mobile/order_reversal_step_up_verify';
$route['pos-mobile/orders/reprint-step-up/verify'] = 'pos_mobile/order_reprint_step_up_verify';
$route['pos-mobile/orders/save'] = 'pos_mobile/order_save';
$route['pos-mobile/orders/confirm'] = 'pos_mobile/order_confirm';
$route['pos-mobile/orders/void/save'] = 'pos_mobile/order_void_save';
$route['pos-mobile/orders/refund/save'] = 'pos_mobile/order_refund_save';
$route['pos-mobile/orders/void/print-targets/(:num)'] = 'pos_mobile/order_void_print_targets/$1';
$route['pos-mobile/orders/refund/print-targets/(:num)'] = 'pos_mobile/order_refund_print_targets/$1';
$route['pos-mobile/orders/reprint-targets/(:num)'] = 'pos_mobile/order_reprint_targets/$1';
$route['pos-mobile/orders/confirm/print-targets/(:num)'] = 'pos_mobile/order_confirm_print_targets/$1';
$route['pos-mobile/orders/push'] = 'pos_mobile/orders_push';
$route['pos-mobile/orders/payment/prepare/(:num)'] = 'pos_mobile/payment_prepare/$1';
$route['pos-mobile/orders/payment/voucher-search'] = 'pos_mobile/voucher_search';
$route['pos-mobile/orders/payment/save'] = 'pos_mobile/payment_save';
$route['pos-mobile/orders/payment/print-targets/(:num)'] = 'pos_mobile/payment_print_targets/$1';
$route['pos/order-monitor'] = 'pos/order_monitor';
$route['pos/order-monitor/data'] = 'pos/order_monitor_data';
$route['pos/order-monitor/ack-task'] = 'pos/order_monitor_ack_task';
$route['pos/order-monitor/ready-task'] = 'pos/order_monitor_ready_task';
$route['pos/order-monitor/checker-task'] = 'pos/order_monitor_checker_task';
$route['pos/order-monitor/ack-order-station'] = 'pos/order_monitor_ack_order_station';
$route['pos/order-monitor/ready-order-station'] = 'pos/order_monitor_ready_order_station';
$route['pos/order-monitor/checker-order'] = 'pos/order_monitor_checker_order';
$route['pos/stock-live'] = 'pos/stock_live';
$route['pos/stock-live/data'] = 'pos/stock_live_data';
$route['pos/stock-live/probe'] = 'pos/stock_live_probe';
$route['pos/stock-live/rebuild'] = 'pos/stock_live_rebuild';
$route['pos/stock-live/rebuild-all'] = 'pos/stock_live_rebuild_all';
$route['pos/availability-queue'] = 'pos/availability_queue';
$route['pos/availability-queue/process'] = 'pos/availability_queue_process';
$route['pos/availability-queue/retry/(:num)'] = 'pos/availability_queue_retry/$1';
$route['pos/orders/draft'] = 'pos/order_draft';
$route['pos/orders/draft/data'] = 'pos/order_draft_data';
$route['pos/orders/draft/load/(:num)'] = 'pos/order_draft_load/$1';
$route['pos/orders/draft/member-search'] = 'pos/order_draft_member_search';
$route['pos/orders/draft/product-search'] = 'pos/order_draft_product_search';
$route['pos/orders/draft/bundle-search'] = 'pos/order_draft_bundle_search';
$route['pos/orders/draft/extra-options'] = 'pos/order_draft_extra_options';
$route['pos/orders/draft/save'] = 'pos/order_draft_save';
$route['pos/orders/draft/save-confirm'] = 'pos/order_draft_save_confirm';
$route['pos/orders/draft/confirm/(:num)'] = 'pos/order_draft_confirm/$1';
$route['pos/orders/draft/delete/(:num)'] = 'pos/order_draft_delete/$1';
$route['pos/orders/payment/prepare/(:num)'] = 'pos/order_payment_prepare/$1';
$route['pos/orders/payment/voucher-search'] = 'pos/order_payment_voucher_search';
$route['pos/orders/payment/save'] = 'pos/order_payment_save';
$route['pos/orders/payment/print-targets/(:num)'] = 'pos/order_payment_print_targets/$1';
$route['pos/orders/receipt-print-targets/(:num)'] = 'pos/order_receipt_print_targets/$1';
$route['pos/orders/confirm-print-targets/(:num)'] = 'pos/order_confirm_print_targets/$1';
$route['pos/orders/reprint-step-up/verify'] = 'pos/order_reprint_step_up_verify';
$route['pos/orders/reprint-print-targets/(:num)'] = 'pos/order_reprint_print_targets/$1';
$route['pos/orders/void-print-targets/(:num)'] = 'pos/order_void_print_targets/$1';
$route['pos/orders/refund-print-targets/(:num)'] = 'pos/order_refund_print_targets/$1';
$route['pos/orders/runtime-jobs/failed'] = 'pos/order_runtime_failed_jobs';
$route['pos/orders/runtime-jobs/active'] = 'pos/order_runtime_active_jobs';
$route['pos/orders/runtime-jobs/trigger/(:num)'] = 'pos/order_runtime_job_trigger/$1';
$route['pos/orders/runtime-jobs/process-all'] = 'pos/order_runtime_jobs_process_all';
$route['pos/orders/runtime-jobs/retry/(:num)'] = 'pos/order_runtime_job_retry/$1';
$route['pos/orders/runtime-jobs/retry-failed-all'] = 'pos/order_runtime_failed_jobs_retry_all';
$route['pos/orders/runtime-jobs/delete-draft/(:num)'] = 'pos/order_runtime_failed_job_delete_draft/$1';
$route['pos/orders/runtime-jobs/dismiss/(:num)'] = 'pos/order_runtime_failed_job_dismiss/$1';
$route['pos/orders/runtime-jobs/status/(:num)'] = 'pos/order_runtime_job_status/$1';
$route['pos/orders/runtime-snapshots/retry/(:num)'] = 'pos/order_runtime_failed_snapshot_retry/$1';
$route['pos/orders/runtime-snapshots/dismiss/(:num)'] = 'pos/order_runtime_failed_snapshot_dismiss/$1';
$route['pos/orders/runtime-sync/(:num)'] = 'pos/order_runtime_sync/$1';
$route['pos/orders/paid'] = 'pos/order_paid';
$route['pos/orders/reversal-preview/(:num)'] = 'pos/order_reversal_preview/$1';
$route['pos/orders/reversal-step-up/verify'] = 'pos/order_reversal_step_up_verify';
$route['pos/orders/void/save'] = 'pos/order_void_save';
$route['pos/orders/refund/save'] = 'pos/order_refund_save';
$route['pos/reports/sales'] = 'pos/report_sales';
$route['pos/reports/cost-control'] = 'pos/report_cost_control';
$route['pos/reports/sales/print/(:num)/(:any)'] = 'pos/report_sales_document_print/$1/$2';
$route['pos/reports/sales/(:num)'] = 'pos/report_sales_transaction/$1';
$route['pos/reports/daily-sales'] = 'pos/report_daily_sales';
$route['pos/reports/daily-sales/print'] = 'pos/report_daily_sales_print';
$route['pos/reports/sales/payment-line/update/(:num)'] = 'pos/report_sales_payment_line_update/$1';
$route['pos/reports/sales-detail'] = 'pos/report_sales_detail';
$route['pos/reports/sales-detail/(:num)'] = 'pos/report_sales_transaction/$1';
$route['pos/reports/sales-extra'] = 'pos/report_sales_extra';
$route['pos/reports/sales-audit'] = 'pos/report_sales_audit';
$route['finance-reports/control'] = 'finance_insights/index';
$route['finance-reports/accounting'] = 'finance_accounting/index';
$route['finance-reports/accounting/post'] = 'finance_accounting/post';
$route['finance-reports/accounting/settings/account'] = 'finance_accounting/save_setup/account';
$route['finance-reports/accounting/settings/mapping'] = 'finance_accounting/save_setup/mapping';
$route['finance-reports/control/lookup/(:any)'] = 'finance_insights/lookup/$1';
$route['finance-reports/control/evidence/upload'] = 'finance_insights/evidence_upload';
$route['finance-reports/control/evidence/(:num)'] = 'finance_insights/evidence_download/$1';
$route['finance-reports/control/save/(:any)'] = 'finance_insights/save/$1';
$route['pos/reports/payments'] = 'pos/report_payments';
$route['pos/reports/payments/(:num)'] = 'pos/report_payment_detail/$1';
$route['pos/reports/payment-methods'] = 'pos/report_payment_methods';
$route['pos/reports/payment-accounts'] = 'pos/report_payment_accounts';
$route['pos/reports/cashier-close'] = 'pos/report_cashier_close';
$route['pos/reports/cashier-close/(:num)'] = 'pos/report_cashier_close_detail/$1';
$route['pos/reports/refunds'] = 'pos/report_refunds';
$route['pos/reports/refunds/(:num)'] = 'pos/report_refund_detail/$1';
$route['pos/reports/voids'] = 'pos/report_voids';
$route['pos/reports/voids/(:num)'] = 'pos/report_void_detail/$1';
$route['inventory/stock/lot/usage/(:num)'] = 'purchase/material_lot_usage/$1';
$route['production/component-lots/usage/(:num)'] = 'production/component_lot_usage/$1';
$route['pos/outlets/data'] = 'pos/outlets_data';
$route['pos/outlets/save'] = 'pos/outlet_save';
$route['pos/outlets/toggle/(:num)'] = 'pos/outlet_toggle/$1';
$route['pos/terminals/data'] = 'pos/terminals_data';
$route['pos/terminals/save'] = 'pos/terminal_save';
$route['pos/terminals/toggle/(:num)'] = 'pos/terminal_toggle/$1';

// Users
$route['users']                   = 'users/index';
$route['users/access-audit']      = 'users/access_audit';
$route['users/access-audit/(:num)'] = 'users/access_audit/$1';
$route['users/create']            = 'users/create';
$route['users/store']             = 'users/store';
$route['users/edit/(:num)']       = 'users/edit/$1';
$route['users/update/(:num)']     = 'users/update/$1';
$route['users/toggle/(:num)']     = 'users/toggle/$1';
$route['users/permissions/(:num)']= 'users/permissions/$1';
$route['users/save_override/(:num)'] = 'users/save_override/$1';

// Roles
$route['roles']                = 'roles/index';
$route['roles/create']         = 'roles/create';
$route['roles/store']          = 'roles/store';
$route['roles/edit/(:num)']    = 'roles/edit/$1';
$route['roles/update/(:num)']  = 'roles/update/$1';
$route['roles/delete/(:num)']  = 'roles/delete/$1';
$route['roles/matrix/(:num)']  = 'roles/matrix/$1';
$route['roles/save_matrix/(:num)']  = 'roles/save_matrix/$1';
$route['roles/users/(:num)']        = 'roles/users/$1';
$route['roles/save_users/(:num)']   = 'roles/save_users/$1';
$route['roles/matrix-groups']       = 'roles/matrix_groups';
$route['roles/quick-register-menu'] = 'roles/quick_register_menu';
$route['roles/deactivate-menu']     = 'roles/deactivate_menu_item';
$route['roles/deactivate-page']     = 'roles/deactivate_page_item';
$route['roles/toggle-page-active']  = 'roles/toggle_page_active';
$route['roles/save-page-group']     = 'roles/save_page_matrix_group';

// Sidebar favorites (AJAX)
$route['sidebar/pin']   = 'sidebar/pin';
$route['sidebar/unpin'] = 'sidebar/unpin';
$route['sidebar/reorder'] = 'sidebar/reorder';
$route['sidebar/manage'] = 'sidebar/manage';
$route['sidebar/manage/save'] = 'sidebar/save_structure';
$route['sidebar/manage/menu/store'] = 'sidebar/menu_store';
$route['sidebar/manage/menu/update/(:num)'] = 'sidebar/menu_update/$1';
$route['sidebar/manage/menu/delete/(:num)'] = 'sidebar/menu_delete/$1';
$route['sidebar/manage/menu/toggle/(:num)'] = 'sidebar/menu_toggle_active/$1';

// Master Data Tahap 2
$route['master']                         = 'master/index/uom';
$route['master/relation/product-recipe']                         = 'master_relation/product_recipe_hub';
$route['master/relation/product-recipe/(:num)']                 = 'master_relation/product_recipe/$1';
$route['master/relation/product-recipe/source-lookup/(:num)']   = 'master_relation/product_recipe_source_lookup/$1';
$route['master/relation/product-recipe/edit-all/(:num)']        = 'master_relation/product_recipe_bulk_edit/$1';
$route['master/relation/product-recipe/edit-all/(:num)/save']   = 'master_relation/product_recipe_bulk_save/$1';
$route['master/relation/product-recipe/(:num)/create']          = 'master_relation/product_recipe_create/$1';
$route['master/relation/product-recipe/(:num)/store']           = 'master_relation/product_recipe_store/$1';
$route['master/relation/product-recipe/edit/(:num)']            = 'master_relation/product_recipe_edit/$1';
$route['master/relation/product-recipe/edit/(:num)/update']     = 'master_relation/product_recipe_update/$1';
$route['master/relation/product-recipe/delete/(:num)']          = 'master_relation/product_recipe_delete/$1';
$route['master/product/hpp-stock/(:num)']                        = 'master_relation/product_hpp_stock/$1';
$route['product/availability']                                  = 'master_relation/product_availability';

$route['master/relation/component-formula']                      = 'master_relation/component_formula_hub';
$route['master/relation/component-formula/(:num)']              = 'master_relation/component_formula/$1';
$route['master/relation/component-formula/(:num)/create']       = 'master_relation/component_formula_create/$1';
$route['master/relation/component-formula/(:num)/store']        = 'master_relation/component_formula_store/$1';
$route['master/relation/component-formula/edit/(:num)']         = 'master_relation/component_formula_edit/$1';
$route['master/relation/component-formula/edit/(:num)/update']  = 'master_relation/component_formula_update/$1';
$route['master/relation/component-formula/delete/(:num)']       = 'master_relation/component_formula_delete/$1';

$route['master/relation/product-extra']                         = 'master_relation/product_extra_hub';
$route['master/relation/product-extra-workspace']               = 'master_relation/extra_workspace';
$route['master/relation/product-extra/(:num)']                  = 'master_relation/product_extra/$1';
$route['master/relation/product-extra/(:num)/create']           = 'master_relation/product_extra_create/$1';
$route['master/relation/product-extra/(:num)/store']            = 'master_relation/product_extra_store/$1';
$route['master/relation/product-extra/delete/(:num)']           = 'master_relation/product_extra_delete/$1';
$route['master/relation/product-bundle']                        = 'master_relation/product_bundle_hub';
$route['master/relation/product-bundle/create']                 = 'master_relation/product_bundle_create';
$route['master/relation/product-bundle/create/save']            = 'master_relation/product_bundle_store';
$route['master/relation/product-bundle/(:num)']                 = 'master_relation/product_bundle/$1';
$route['master/relation/product-bundle/edit/(:num)']            = 'master_relation/product_bundle_edit/$1';
$route['master/relation/product-bundle/edit/(:num)/save']       = 'master_relation/product_bundle_update/$1';
$route['master/relation/product-bundle/toggle/(:num)']          = 'master_relation/product_bundle_toggle/$1';
$route['master/relation/product-bundle/product-search']         = 'master_relation/product_bundle_product_search';

$route['master/relation/extra-group']                           = 'master_relation/extra_group_hub';
$route['master/relation/extra-group/(:num)/extras']             = 'master_relation/extra_group_items_ajax/$1';
$route['master/relation/extra-group/(:num)/extras/save']        = 'master_relation/extra_group_items_save_ajax/$1';
$route['master/relation/extra-group/(:num)/products']           = 'master_relation/extra_group_products_ajax/$1';
$route['master/relation/extra-group/(:num)/products/save']      = 'master_relation/extra_group_products_save_ajax/$1';
$route['master/relation/extra-group/(:num)']                    = 'master_relation/extra_group_products/$1';
$route['master/relation/extra-group/(:num)/save']               = 'master_relation/extra_group_products_save/$1';
$route['master/relation/extra-item-group']                      = 'master_relation/extra_item_group_hub';
$route['master/relation/extra-item-group/(:num)']               = 'master_relation/extra_item_groups/$1';
$route['master/relation/extra-item-group/(:num)/save']          = 'master_relation/extra_item_groups_save/$1';
$route['master/att-holiday/generate-year']      = 'master/att_holiday_generate_year';

$route['master/material/usage/(:num)']          = 'master/material_usage/$1';

$route['dbtools']                               = 'system_tools/index';
$route['dbtools/backup-guide']                  = 'system_tools/index';
$route['dbtools/replication-guide']             = 'system_tools/index';
$route['dbtools/settings']                      = 'system_tools/index';
$route['dbtools/backup-status']                 = 'system_tools/backup_status';
$route['dbtools/replication-status']            = 'system_tools/replication_status';
$route['dbtools/settings/save']                 = 'system_tools/settings_save';
$route['dbtools/action/list-tables']            = 'system_tools/action_list_tables';
$route['dbtools/action/run-backup']             = 'system_tools/action_run_backup';
$route['dbtools/action/test-db']                = 'system_tools/action_test_db';
$route['dbtools/action/check-replication']      = 'system_tools/action_check_replication';
$route['dbtools/action/apply-mysql-config']      = 'system_tools/action_apply_mysql_config';
$route['dbtools/action/initial-sync']            = 'system_tools/action_initial_sync';
$route['dbtools/action/compare-data']            = 'system_tools/action_compare_data';
$route['dbtools/action/setup-master']            = 'system_tools/action_setup_master';
$route['dbtools/action/failover']               = 'system_tools/action_failover';
$route['dbtools/action/restart-replication']    = 'system_tools/action_restart_replication';
$route['system/activity-audit']                 = 'activity_audit/index';
$route['system/business-profile']               = 'business_profile/index';
$route['system/roast-connect'] = 'roast_integrations/index';
$route['system/roast-connect/save'] = 'roast_integrations/save';
$route['system/roast-connect/token'] = 'roast_integrations/rotate';
$route['system/license']                        = 'license/index';
$route['system/updates']                        = 'application_update/index';
$route['system/updates/confirm']                = 'application_update/confirm';

$route['purchase/item-price-history']           = 'purchase/item_price_history';
$route['purchase/item-price-history/item-search'] = 'purchase/item_price_history_item_search';
$route['purchase/item-price-history/data']      = 'purchase/item_price_history_data';
$route['purchase/item-price-history/(:num)']    = 'purchase/item_price_history/$1';

// Landing Page Settings
$route['landing-page']                        = 'landing_page/index';
$route['landing-page/config/update']          = 'landing_page/config_update';
$route['landing-page/menu/store']             = 'landing_page/menu_store';
$route['landing-page/menu/update/(:num)']     = 'landing_page/menu_update/$1';
$route['landing-page/menu/delete/(:num)']     = 'landing_page/menu_delete/$1';
$route['landing-page/menu/toggle/(:num)']     = 'landing_page/menu_toggle/$1';
$route['landing-page/menu/reorder']           = 'landing_page/menu_reorder';
$route['landing-page/gallery/store']          = 'landing_page/gallery_store';
$route['landing-page/gallery/update/(:num)']  = 'landing_page/gallery_update/$1';
$route['landing-page/gallery/delete/(:num)']  = 'landing_page/gallery_delete/$1';
$route['landing-page/gallery/toggle/(:num)']  = 'landing_page/gallery_toggle/$1';
$route['landing-page/gallery/reorder']        = 'landing_page/gallery_reorder';
$route['landing-page/embed/store']            = 'landing_page/embed_store';
$route['landing-page/embed/update/(:num)']    = 'landing_page/embed_update/$1';
$route['landing-page/embed/delete/(:num)']    = 'landing_page/embed_delete/$1';
$route['landing-page/embed/toggle/(:num)']    = 'landing_page/embed_toggle/$1';
$route['landing-page/links/store']            = 'landing_page/links_store';
$route['landing-page/links/update/(:num)']    = 'landing_page/links_update/$1';
$route['landing-page/links/delete/(:num)']    = 'landing_page/links_delete/$1';
$route['landing-page/links/toggle/(:num)']    = 'landing_page/links_toggle/$1';
$route['landing-page/links/reorder']          = 'landing_page/links_reorder';

// WhatsApp Module
$route['wa']                              = 'whatsapp/dashboard';
$route['wa/dashboard']                    = 'whatsapp/dashboard';
$route['wa/broadcast']                    = 'whatsapp/broadcast';
$route['wa/broadcast/create']             = 'whatsapp/broadcast_create';
$route['wa/broadcast/edit/(:num)']        = 'whatsapp/broadcast_edit/$1';
$route['wa/broadcast/detail/(:num)']      = 'whatsapp/broadcast_detail/$1';
$route['wa/broadcast/deactivate/(:num)']  = 'whatsapp/broadcast_deactivate/$1';
$route['wa/broadcast/delete/(:num)']      = 'whatsapp/broadcast_delete/$1';
$route['wa/template']                     = 'whatsapp/template';
$route['wa/template/schedules']           = 'whatsapp/report_schedules';
$route['wa/group']                        = 'whatsapp/group';
$route['wa/log']                          = 'whatsapp/log';
$route['wa/manual']                       = 'whatsapp/manual';
$route['wa/settings']                     = 'whatsapp/settings';
$route['wa/api/status']                   = 'whatsapp/api_status';
$route['wa/api/send-test']                = 'whatsapp/api_send_test';
$route['wa/api/log-retry/(:num)']         = 'whatsapp/api_log_retry/$1';
$route['wa/api/member-search']            = 'whatsapp/api_member_search';
$route['wa/api/member-picker']            = 'whatsapp/api_member_picker';
$route['wa/api/broadcast-start/(:num)']   = 'whatsapp/api_broadcast_start/$1';
$route['wa/api/template-preview']         = 'whatsapp/api_template_preview';
$route['wa/api/schedule-run']             = 'whatsapp/api_schedule_run';
$route['wa/api/group-command']            = 'whatsapp/api_group_command';
$route['wa/api/qr']                       = 'whatsapp/api_qr';
$route['wa/api/engine-status']            = 'whatsapp/api_engine_status';
$route['wa/api/engine-start']             = 'whatsapp/api_engine_start';
$route['wa/api/engine-stop']              = 'whatsapp/api_engine_stop';
$route['wa/api/engine-logs']              = 'whatsapp/api_engine_logs';
$route['wa/api/env-read']                 = 'whatsapp/api_env_read';
$route['wa/api/env-save']                 = 'whatsapp/api_env_save';
$route['wa/api/session-reset']            = 'whatsapp/api_session_reset';
$route['wa/guide']                        = 'whatsapp/guide';

$route['master/(:any)']                  = 'master/index/$1';
$route['master/(:any)/create']           = 'master/create/$1';
$route['master/(:any)/detail/(:num)']    = 'master/detail/$1/$2';
$route['master/(:any)/reorder']          = 'master/reorder/$1';
$route['master/(:any)/store']            = 'master/store/$1';
$route['master/(:any)/edit/(:num)']      = 'master/edit/$1/$2';
$route['master/(:any)/update/(:num)']    = 'master/update/$1/$2';
$route['master/(:any)/stock-mode/(:num)'] = 'master/stock_mode/$1/$2';
$route['master/(:any)/toggle/(:num)']    = 'master/toggle/$1/$2';
