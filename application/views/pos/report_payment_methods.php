...
<div class="container-xxl py-3">
  <div class="pos-report-shell">
    <div class="pos-report-hero mb-3">
      <div class="pos-report-title">Laporan Metode Pembayaran POS</div>
      <p class="pos-report-copy mb-0">Rekap penerimaan POS per metode pembayaran, termasuk pengurang refund pada rentang tanggal yang dipilih.</p>
    </div>
    <?php $this->load->view('pos/_report_nav', ['report_nav_active' => 'payment_methods']); ?>

    <div class="pos-report-section p-3 mb-3">
      <form method="get" class="pos-report-filter-box row g-2 align-items-end">
        <div class="col-lg-4 col-md-6"><label class="form-label small text-muted mb-1">Cari</label><input type="text" name="q" class="form-control" value="<?php echo html_escape((string)($filters['q'] ?? '')); ?>" placeholder="Metode / payment / order / member"></div>
        ...
        <div class="col-lg-2 col-md-8 d-flex gap-2"><button class="btn btn-dark flex-fill">Tampilkan</button><a href="<?php echo site_url('pos/reports/payment-methods'); ?>" class="btn btn-outline-secondary">Reset</a></div>
      </form>
    </div>
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-md-4"><div class="pos-report-card"><div class="pos-report-card-label">Net Metode</div><div class="pos-report-card-value"><?php echo $money($overview['net_amount'] ?? 0); ?></div><div class="pos-report-card-note">Masuk dikurangi refund.</div></div></div>
    </div>

    <div class="pos-report-section p-3">
      <div class="table-responsive">
        <table class="table table-sm align-middle mb-0 pos-report-table">
          <thead><tr><th>Metode</th><th>Tipe</th><th class="text-center">Baris</th><th class="text-center">Transaksi</th><th class="text-end">Masuk</th><th class="text-end">Refund</th><th class="text-end">Net</th></tr></thead>
          <tbody>
          <?php if (empty($rows)): ?>
            <tr><td colspan="7" class="text-center pos-report-empty">Data tidak ditemukan.</td></tr>
          <?php else: foreach ($rows as $row): ?>
            <tr>
              <td class="fw-semibold"><?php echo html_escape((string)($row['method_name'] ?? '-')); ?></td>
              <td><?php echo html_escape((string)($row['method_type'] ?? '-')); ?></td>
