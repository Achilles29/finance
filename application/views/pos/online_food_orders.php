...
<div class="container-xxl py-3">
  ...
  <?php $this->load->view('pos/_online_food_tabs', ['online_food_tab_active' => 'orders']); ?>

  <div class="fin-page-header">
    <div>
      <h4 class="fin-page-title mb-1">Orderan Online Food</h4>
      <p class="fin-page-subtitle mb-0">Pantau order delivery dari member, verifikasi, lalu cetak KOT sesuai pengaturan outlet.</p>
    </div>
  </div>
  ...
  <div class="self-order-shell">
    <div class="card self-order-card">
      <div class="card-body p-4">
        <div class="self-order-filter-card mb-3">
          <div class="row g-3 align-items-end">
            <div class="col-lg-4">
              ...
              <input type="text" class="form-control" id="self_order_q" placeholder="Order no, customer, nomor HP, channel, atau metode bayar">
            </div>
        ...
        <div id="self_order_empty_state" class="self-order-empty d-none" role="status" aria-live="polite">Belum ada order online food pada filter ini.</div>
        ...
        <div class="d-flex justify-content-between align-items-center mt-3">
          ...
          <div class="d-flex gap-1" id="self_order_pagination" aria-label="Navigasi halaman online food"></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal-dialog modal-xl modal-dialog-scrollable">
    <div class="modal-content border-0 shadow-lg" style="border-radius:24px;">
      <div class="modal-header">
        <div>
          <h5 class="modal-title mb-1">Detail Order Online Food</h5>
...
<div class="modal fade" id="selfOrderVerifyModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content border-0 shadow-lg" style="border-radius:24px;">
      <div class="modal-header">
        <div>
          <h5 class="modal-title mb-1">Verifikasi Order Online Food</h5>
...
<div class="modal fade" id="selfOrderRejectModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content border-0 shadow-lg" style="border-radius:24px;">
      <div class="modal-header">
        <div>
          <h5 class="modal-title mb-1">Tolak Order Online Food</h5>
...
(function() {
  ...
  const summaryDefs = [
    { key: 'ALL', label: 'Total Online Food', note: 'Semua order sesuai filter tanggal/outlet.' },
  ...
  function serviceLabel(row) {
    const type = String((row && row.service_type) || '').toUpperCase();
    if (type === 'PICKUP') return 'Ambil sendiri';
    if (type === 'DELIVERY') return 'Dikirim';
    return type || 'Online Food';
  }

  function deliveryChargeLabel(row) {
    const type = String((row && row.service_type) || '').toUpperCase();
    if (type === 'PICKUP') return 'Tanpa ongkir';
    const fee = Number((row && row.delivery_fee_amount) || 0);
    const paidBy = String((row && row.delivery_fee_paid_by) || '').toUpperCase();
    if (fee <= 0 || paidBy === 'FREE') return 'Gratis ongkir';
    const charge = String((row && row.delivery_fee_charge_mode) || '').toUpperCase();
    return charge === 'MERCHANT_COLLECT' ? 'Ongkir ikut total' : 'Ongkir di penerima';
  }
  ...
  async function kickoffRuntimeJobSync(orderId, jobId) {
    ...
    try {
      ...
      showToast('Stok online food sudah tersinkron.', 'success');
    } catch (e) {
  ...
  function paymentModeChip(row) {
    ...
    const label = mode === 'QRIS' ? 'QRIS' : 'Manual Admin';
  ...
  async function loadOrderDetailOnce(orderId) {
    const safeOrderId = Number(orderId || 0);
    if (safeOrderId <= 0) {
      throw new Error('Order online food tidak valid.');
    }
    if (orderDetailCache.has(safeOrderId)) {
      return orderDetailCache.get(safeOrderId);
    }
    const detail = await getJson(`<?php echo site_url('pos/online-food/orders/detail'); ?>/${safeOrderId}`);
  ...
  function renderRows(rows) {
    ...
    if (!Array.isArray(rows) || !rows.length) {
      ...
      empty.innerHTML = '<strong>Belum ada order online food pada filter ini.</strong><div class="small mt-1">Periksa mode pembayaran, status, rentang tanggal, outlet, atau pencarian.</div><button type="button" class="btn btn-sm btn-outline-secondary mt-2" data-online-food-clear>Bersihkan filter</button>';
      empty.querySelector('[data-online-food-clear]')?.addEventListener('click', resetListFilters);
      return;
    }
    ...
    body.innerHTML = rows.map((row) => {
      ...
      const verifyMessage = String(row.verify_message || 'Order online food belum siap diverifikasi.');
      const isRejected = Number(row.is_rejected || 0) === 1;
      const rejectReason = String(row.rejected_reason || '').trim();
      const serviceText = serviceLabel(row);
      const chargeText = deliveryChargeLabel(row);
      return `
        <tr>
          <td>
          ...
          <td>
            ...
            <div class="self-order-subtle">${escapeHtml(serviceText)} | ${escapeHtml(row.member_mobile_phone || '-')}</div>
            <div class="self-order-subtle">${escapeHtml(chargeText)}</div>
          </td>
          <td>
            ${paymentModeChip(row)}
          ...
          <td class="text-end">
            ...
            ${Number(row.delivery_fee_amount || 0) > 0 ? `<div class="self-order-subtle">Ongkir ${money(row.delivery_fee_amount || 0)}</div>` : ''}
    ...
    body.querySelectorAll('.btn-self-order-verify').forEach((btn) => btn.addEventListener('click', () => {
      ...
      if (String(btn.dataset.canVerify || '0') !== '1') {
        showInfoModal(String(btn.dataset.verifyMessage || row.verify_message || 'Order online food belum siap diverifikasi.'), 'Status Verifikasi');
        return;
      }
  ...
  function setListState(type = '', message = '', retry = false) {
    ...
    const retryButton = retry ? '<button type="button" class="btn btn-sm btn-outline-danger ms-2" data-online-food-retry>Coba lagi</button>' : '';
    box.innerHTML = `${spinner}${escapeHtml(message)}${retryButton}`;
    box.querySelector('[data-online-food-retry]')?.addEventListener('click', () => loadRows());
  }
  ...
  async function loadRows() {
    ...
    setListState('loading', 'Memuat order online food...');
    try {
      const json = await getJson(
        '<?php echo site_url('pos/online-food/orders/data'); ?>?' + qs.toString(),
      ...
      setListState('error', error.message || 'Order online food tidak dapat dimuat.', true);
    } finally {
  ...
  async function pollIncomingOrders() {
    ...
    try {
      ...
      qs.set('page', '1');
      qs.set('limit', '20');
      const json = await getJson('<?php echo site_url('pos/online-food/orders/data'); ?>?' + qs.toString());
      const rows = Array.isArray(json.rows) ? json.rows : [];
      // Payments and verification can change an existing ID on another screen.
      ...
      if (newRows.length) {
        ...
        const tableLabel = ` | ${serviceLabel(newest)}`;
  ...
  async function openDetail(orderId) {
    ...
    const deliveryInfo = [
      ...
      ['Mode Order', serviceLabel(header)],
      ['Status Order', header.status || '-'],
      ['Stock Commit', header.stock_commit_status || '-'],
      ['Service', header.service_type || '-'],
      ['Guest', header.guest_count || 1],
      ['Grand Total Menu', money(header.grand_total || 0)],
      ['Kasir', header.cashier_username || header.cashier_employee_name || '-']
    ];
    if (String(header.service_type || '').toUpperCase() === 'DELIVERY') {
      deliveryInfo.push(
        ['Ongkir', money(header.delivery_fee_amount || 0)],
        ['Penagihan Ongkir', deliveryChargeLabel(header)],
        ['Alamat', header.delivery_address || '-'],
        ['Penerima', [header.delivery_recipient_name || '', header.delivery_recipient_phone || ''].filter(Boolean).join(' | ') || '-']
      );
    }
    document.getElementById('self_order_detail_meta').textContent = `${header.order_no || '-'} | ${header.customer_display_name || header.member_name || 'Walk in'} | ${serviceLabel(header)}`;
    document.getElementById('self_order_detail_grid').innerHTML = deliveryInfo.map((item) => `<div class="self-order-detail-item"><div class="self-order-detail-label">${escapeHtml(item[0])}</div><div class="self-order-detail-value">${escapeHtml(item[1])}</div></div>`).join('');
    const detailNotes = [];
  ...
  async function openVerify(row) {
    ...
    document.getElementById('self_order_verify_meta').textContent = `${row.order_no || '-'} | ${row.customer_name_display || 'Walk in'} | ${row.payment_mode === 'QRIS' ? 'QRIS' : 'Manual Admin'}`;
    if (verifyLinesEl) {
  ...
  function syncVerifyDestinationSummary() {
    ...
    document.getElementById('self_order_verify_grid').innerHTML = [
      ...
      ['Mode Order', serviceLabel(verifyRow)],
      ['Ongkir', deliveryChargeLabel(verifyRow)],
      ['Pembayaran', verifyRow.payment_mode === 'QRIS' ? 'QRIS' : 'Manual Admin'],
      ['Status Bayar', verifyRow.payment_status || '-'],
      ['Nominal Menu', money(verifyRow.grand_total || 0)],
  ...
  function openReject(row) {
    rejectRow = row;
    document.getElementById('self_order_reject_meta').textContent = `${row.order_no || '-'} | ${row.customer_name_display || 'Walk in'} | ${row.payment_mode === 'QRIS' ? 'QRIS' : 'Manual Admin'}`;
    document.getElementById('self_order_reject_grid').innerHTML = [
      ['Customer', row.customer_name_display || 'Walk in'],
      ['Mode Order', serviceLabel(row)],
      ['Ongkir', deliveryChargeLabel(row)],
      ['Pembayaran', row.payment_mode === 'QRIS' ? 'QRIS' : 'Manual Admin'],
      ['Status Bayar', row.payment_status || '-'],
      ['Nominal Menu', money(row.grand_total || 0)],
  ...
  async function submitVerify() {
    ...
    try {
      const json = await postPosTransactionJson(`<?php echo site_url('pos/online-food/orders/verify'); ?>/${Number(verifyRow.id || 0)}`, {
      ...
      showToast(
        String(json.workspace_bucket || '') === 'PAID_ORDER'
          ? 'Order online food berhasil diverifikasi dan masuk ke pesanan terbayar.'
          : 'Order online food berhasil diverifikasi dan masuk ke order aktif.',
        'success'
      );
      ...
      if (printFailures.length) {
        ...
        showInfoModal(message, 'Printer Online Food');
      }

      await loadRows();
    } catch (e) {
      showInfoModal(e && e.message ? e.message : 'Gagal memverifikasi order online food.', 'Verifikasi Gagal');
    } finally {
      verifyBusy = false;
      btn.disabled = false;
  ...
  async function submitReject() {
    ...
    btn.innerHTML = '<span class="self-order-btn-spinner" aria-hidden="true"></span> Memproses';
    try {
      await postPosTransactionJson(`<?php echo site_url('pos/online-food/orders/reject'); ?>/${Number(rejectRow.id || 0)}`, { reason });
      hideModal(rejectModalEl, rejectModal);
      showToast('Order online food berhasil ditolak.', 'warning');
      state.payment_tab = 'ALL';
      state.status_tab = 'REJECTED';
      state.page = 1;
      await loadRows();
    } catch (e) {
      showInfoModal(e && e.message ? e.message : 'Gagal menolak order online food.', 'Penolakan Gagal');
    } finally {
      rejectBusy = false;
      btn.disabled = false;
      btn.innerHTML = originalHtml;
