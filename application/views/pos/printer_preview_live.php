...
?>
<style>
  .live-preview-stage{min-height:600px;display:flex;align-items:center;justify-content:center;padding:2.25rem 1.25rem;background:radial-gradient(circle at 50% 10%,#4b525c 0,#24272d 53%,#121417 100%);border-radius:16px;overflow:auto}
  .live-preview-paper{--preview-paper-width:440px;--preview-font-size:13px;box-sizing:border-box;width:min(100%,var(--preview-paper-width));min-height:430px;background:#fff;color:#1d2025;box-shadow:0 22px 40px rgba(0,0,0,.42);padding:1rem;transition:width .16s ease}
  ...
  .live-preview-qr .qr-copy{max-width:190px;line-height:1.4}.live-preview-qr img{width:88px;height:88px;flex:0 0 auto;display:block;padding:4px;border:1px solid #d9c3b8;background:#fff;border-radius:5px}
  ...
  async function load() {
    ...
    try {
      ...
      if (reviewQr.enabled) {
        ...
        if (qrUrl !== '') { previewQrImage.src = 'https://api.qrserver.com/v1/create-qr-code/?size=240x240&ecc=H&data=' + encodeURIComponent(qrUrl); previewQrImage.classList.remove('d-none'); } else { previewQrImage.removeAttribute('src'); previewQrImage.classList.add('d-none'); previewQrMessage.textContent += ' (QR area belum tersedia pada database.)'; }
        previewQr.classList.remove('d-none');
       } else { previewQr.classList.add('d-none'); previewQrImage.classList.add('d-none'); previewQrImage.removeAttribute('src'); previewQrMessage.textContent = ''; }
