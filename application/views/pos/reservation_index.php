...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function postPosTransactionJson(url, payload) { const response=await fetch(url,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest','X-Pos-Transaction-CSRF':posTransactionCsrfToken},body:JSON.stringify(payload || {})}); const text=await response.text(); let data; try { data=JSON.parse(text); } catch(error) { throw new Error('Respons server bukan JSON valid. Periksa login, permission, atau log aplikasi.'); } if(!response.ok || !data.ok) throw new Error(data.message || 'Permintaan gagal diproses.'); return data; }
  ...
  async function saveReservation() { const payload=editorPayload();if(!payload.outlet_id)throw new Error('Outlet reservasi wajib dipilih.');if(!payload.customer_name.trim())throw new Error('Nama customer wajib diisi.');if(!payload.reservation_at)throw new Error('Tanggal dan jam reservasi wajib diisi.');if(!payload.lines.length)throw new Error('Pilih minimal satu produk.');if(payload.deposit.amount>0 && !payload.deposit.payment_method_id)throw new Error('Pilih metode pembayaran untuk DP.');if(payload.deposit.amount>cartTotal()+.009)throw new Error('DP tidak boleh melebihi total tagihan.');const data=await postPosTransactionJson(urls.save,payload);notify(`Reservasi ${data.reservation_no||''} berhasil disimpan.`, 'success');editorModal && editorModal.hide();await loadRows();return data; }
  ...
  async function saveMoreDeposit() { const id=state.currentReservationId;const payload={deposit:{amount:Number(el('reservation_more_deposit_amount').value||0),payment_method_id:Number(el('reservation_more_deposit_method').value||0),reference_no:el('reservation_more_deposit_reference').value}};if(!id)throw new Error('Reservasi belum dipilih.');if(payload.deposit.amount<=0)throw new Error('Nominal DP wajib lebih besar dari nol.');if(!payload.deposit.payment_method_id)throw new Error('Pilih metode pembayaran DP.');await postPosTransactionJson(`${urls.deposit}/${id}`,payload);depositModal&&depositModal.hide();notify('DP reservasi berhasil diterima dan masuk ke mutasi rekening.','success');await loadRows(); }
  async function verifyReservation(id) { if(!window.confirm('Terima reservasi ini ke POS? Stok baru diproses setelah order POS terbentuk dan printer akan mengikuti aturan cetak yang aktif.'))return;const data=await postPosTransactionJson(`${urls.verify}/${id}`,{});await kickoffRuntime(data);const printing=await directPrintTargets(data.direct_print_targets||[]);if(printing.failed.length)notify(`Order POS berhasil dibuat, tetapi printer perlu dicek: ${printing.failed.join('; ')}`,'warning');else notify(data.recovered_finalization?'Status reservasi berhasil dipulihkan ke POS.':'Reservasi diterima dan berhasil masuk ke POS.','success');detailModal&&detailModal.hide();await loadRows(); }
  ...
  async function saveClose() { const id=state.currentReservationId;const reason=el('reservation_close_reason').value.trim();if(!reason)throw new Error('Alasan wajib diisi.');const endpoint=state.closeMode==='reject'?urls.reject:urls.cancel;await postPosTransactionJson(`${endpoint}/${id}`,{reason,refund_deposit:el('reservation_close_refund').checked});closeModal&&closeModal.hide();detailModal&&detailModal.hide();notify(state.closeMode==='reject'?'Reservasi ditolak dengan jejak audit yang lengkap.':'Reservasi dibatalkan dengan jejak audit yang lengkap.','success');await loadRows(); }
  ...
  async function kickoffRuntime(data){const orderId=Number(data.id||0);const jobId=Number(data.runtime_job_id||0);if(!orderId)return;try{if(jobId>0)await postPosTransactionJson(`${urls.orderTrigger}/${orderId}`,{job_id:jobId,limit:1});await request(`${urls.orderSync}/${orderId}`,'POST',{event_source:'RESERVATION_VERIFY',event_id:Number(data.reservation_id||0)});}catch(error){notify(`Order sudah dibuat. Sinkron stok berjalan di antrean: ${error.message}`,'warning');}}
