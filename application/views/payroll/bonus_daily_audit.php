...
$backUrl = $back_url ?? site_url('payroll/bonus');
?>

<style>
  .bonus-audit-card { border:1px solid rgba(122,24,36,.08); border-radius:22px; box-shadow:0 16px 34px rgba(89,57,41,.06); }
  .bonus-audit-summary { display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:1rem; }
  .bonus-audit-box { border:1px solid rgba(122,24,36,.08); border-radius:18px; padding:1rem 1.05rem; background:linear-gradient(180deg,#fff,#fff8f5); }
  .bonus-audit-box .label { font-size:.76rem; text-transform:uppercase; letter-spacing:.04em; color:#8d7368; margin-bottom:.35rem; display:block; }
  .bonus-audit-box .value { color:#5f1720; font-weight:800; }
  .bonus-audit-table-wrap { max-height:68vh; overflow:auto; border:1px solid rgba(122,24,36,.08); border-radius:18px; }
  .bonus-audit-table-wrap thead th { position:sticky; top:0; z-index:2; background:linear-gradient(135deg,#b11226,#8f1021); color:#fff; border:0; white-space:nowrap; }
  @media (max-width: 991.98px) { .bonus-audit-summary { grid-template-columns:repeat(2, minmax(0,1fr)); } }
  @media (max-width: 575.98px) { .bonus-audit-summary { grid-template-columns:1fr; } }
</style>

<div class="d-flex justify-content-between align-items-center gap-2 flex-wrap mb-3">
  <div>
    <h4 class="mb-1">Audit Bonus Harian</h4>
    <div class="text-muted small">Halaman ini memperlihatkan sumber bonus per irisan waktu transaksi, lalu berapa bagian pegawai dari tiap irisan itu.</div>
  </div>
  <a href="<?php echo html_escape($backUrl); ?>" class="btn btn-outline-secondary">Kembali</a>
</div>

<div class="card bonus-audit-card">
  <div class="card-body">
    <div class="bonus-audit-summary mb-4">
      <div class="bonus-audit-box">
        <span class="label">Pegawai</span>
        <div class="value"><?php echo html_escape((string)($header['employee_name'] ?? '-')); ?></div>
        <div class="small text-muted"><?php echo html_escape((string)($header['employee_code'] ?? '-')); ?> · <?php echo html_escape((string)($header['division_name'] ?? '-')); ?></div>
      </div>
      <div class="bonus-audit-box">
    ...
    <div class="bonus-audit-table-wrap">
      <table class="table align-middle mb-0">
        ...
        <?php if (empty($sliceRows)): ?>
          <tr><td colspan="9" class="text-center text-muted py-4">Belum ada rincian slice tersimpan untuk baris bonus ini.</td></tr>
        <?php else: ?>
