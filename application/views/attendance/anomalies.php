...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h4 class="mb-0"><?php echo html_escape($title ?? 'Monitoring Anomali Absensi'); ?></h4>
  <span class="text-muted small">Total: <?php echo (int)$pg['total']; ?></span>
</div>

<div class="card mb-3">
  <div class="card-body">
    <form method="get" action="<?php echo site_url('attendance/anomalies'); ?>" class="row g-2 align-items-end">
      <div class="col-md-3"><label class="form-label mb-1">Cari</label><input type="text" name="q" class="form-control" value="<?php echo html_escape((string)($filters['q'] ?? '')); ?>" placeholder="Nama/NIP/Divisi"></div>
      ...
      <div class="col-md-3"><label class="form-label mb-1">Jenis Anomali</label><select name="issue_type" class="form-select"><option value="">Semua</option><?php foreach($issueOptions as $o): ?><option value="<?php echo html_escape($o); ?>" <?php echo (($filters['issue_type'] ?? '')===$o)?'selected':''; ?>><?php echo html_escape($o); ?></option><?php endforeach; ?></select></div>
      ...
      <div class="col-12"><button type="submit" class="btn btn-primary">Filter</button> <a class="btn btn-outline-secondary" href="<?php echo site_url('attendance/anomalies'); ?>">Reset</a></div>
    </form>
  </div>
</div>
...
<div class="card">
  <div class="table-responsive">
    <table class="table table-striped mb-0">
      <thead>
        <tr>
          <th>Tanggal</th><th>NIP</th><th>Nama</th><th>Divisi</th><th>Jabatan</th><th>Shift</th><th>Status</th><th>Anomali</th><th>Check-in</th><th>Check-out</th><th class="text-end">Work</th>
        </tr>
      </thead>
      <tbody>
      <?php if (empty($rows)): ?>
        <tr><td colspan="11" class="text-center text-muted py-4">Tidak ada data anomali.</td></tr>
      <?php else: foreach($rows as $r): ?>
        <tr>
          ...
          <td><span class="badge bg-danger"><?php echo html_escape((string)$r['issue_type']); ?></span></td>
  ...
  <div class="card-footer d-flex justify-content-between align-items-center">
    ...
    <div class="btn-group">
      ...
      <a class="btn btn-sm btn-outline-secondary <?php echo ((int)$pg['page']<=1)?'disabled':''; ?>" href="<?php echo ((int)$pg['page']<=1)?'#':site_url('attendance/anomalies?'.$buildQuery(['page'=>$prev])); ?>">&lt;</a>
      <?php foreach ($pageItems as $item): ?>
        <?php if ($item === '...'): ?>
          <span class="btn btn-sm btn-outline-secondary disabled">...</span>
        <?php else: ?>
          <a class="btn btn-sm <?php echo ((int)$pg['page']===(int)$item)?'btn-primary':'btn-outline-secondary'; ?>" href="<?php echo site_url('attendance/anomalies?'.$buildQuery(['page'=>(int)$item])); ?>"><?php echo (int)$item; ?></a>
        <?php endif; ?>
      <?php endforeach; ?>
      <a class="btn btn-sm btn-outline-secondary <?php echo ((int)$pg['page']>=(int)$pg['total_pages'])?'disabled':''; ?>" href="<?php echo ((int)$pg['page']>=(int)$pg['total_pages'])?'#':site_url('attendance/anomalies?'.$buildQuery(['page'=>$next])); ?>">&gt;</a>
    </div>
  </div>
  <?php endif; ?>
</div>
