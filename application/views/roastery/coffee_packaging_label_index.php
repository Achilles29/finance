...
.label-roastery-kicker{position:absolute;z-index:7;left:14%;right:14%;top:20%;text-align:center;font-family:'Space Grotesk',sans-serif;font-size:6.6px;letter-spacing:.26em;text-transform:uppercase;color:rgba(64,36,24,.62);font-weight:900;pointer-events:none;white-space:normal;line-height:1.35}
...
.taste-icon-row{position:absolute;z-index:7;left:18%;right:18%;top:47%;display:flex;align-items:center;justify-content:center;gap:10px;color:#5d3827;font-size:13px;pointer-events:none}
  ...
  <div class="label-workbench mb-4">
      ...
      <div class="label-panel-body">
        <form method="post" action="<?php echo site_url('roastery/packaging-labels/save'); ?>" enctype="multipart/form-data" id="coffeeLabelForm">
          ...
          <div class="label-tools">
            <div class="full"><label class="form-label">Elemen yang diatur</label><select class="form-select" id="blockSelect"><option value="logo">Logo</option><option value="coffee_name">Nama Kopi</option><option value="tasting_notes">Tasting Notes</option><option value="brew_suggestion">Brew Suggestion</option><option value="info_panel">Panel Info Bawah</option></select></div>
    ...
    <div class="label-panel label-preview-card print-target"><div class="label-panel-head"><div><h5>Live Preview</h5><small class="text-muted">Klik teks pada label untuk mengatur bloknya.</small></div><span class="badge bg-label-danger">Roastery</span></div>
      <div class="label-panel-body"><div class="preview-shell"><div id="labelCanvas" class="label-canvas theme-<?php echo html_escape($themePreset); ?> artwork-mode-<?php echo html_escape($artworkMode); ?>" style="--label-preview-w:<?php echo $canvasWidth * 4; ?>px;--label-preview-h:<?php echo $canvasHeight * 4; ?>px;--label-print-w:<?php echo $canvasWidth; ?>mm;--label-print-h:<?php echo $canvasHeight; ?>mm;">
        ...
        <div class="label-roastery-kicker" data-info-value="footer_note"></div>
...
const defaults={canvas:{theme:'heritage-cream',artworkMode:'full'},meta:{body_level:'Light - Medium',elevation_text:'>1200 mdpl',bean_type:'Whole Bean',footer_note:''},tasteIcons:['ri-seedling-line','ri-flower-line','ri-cup-line'],blocks:{logo:{x:14,y:8,w:10,size:24,font:'Jost',color:'#2f2018',bold:true,align:'center',letter:0},coffee_name:{x:12,y:31,w:76,size:31,font:'Cormorant Garamond',color:'#26150f',bold:true,italic:false,uppercase:true,align:'center',letter:4.6},origin:{x:18,y:50,w:64,size:9,font:'Space Grotesk',color:'#3c261c',bold:true,uppercase:true,align:'center',letter:4},process_method:{x:16,y:73,w:22,size:8,font:'Space Grotesk',color:'#3c261c',bold:true,uppercase:true,align:'center',letter:1.2},roast_level:{x:39,y:73,w:22,size:8,font:'Space Grotesk',color:'#3c261c',bold:true,uppercase:true,align:'center',letter:1.2},weight_text:{x:66,y:87,w:22,size:8,font:'Space Grotesk',color:'#3c261c',bold:true,uppercase:true,align:'center',letter:1.1},tasting_notes:{x:15,y:40,w:70,size:7.8,font:'Jost',color:'#5f3c2b',bold:false,italic:false,align:'center',letter:.65},brew_suggestion:{x:18,y:46,w:64,size:6.2,font:'Space Grotesk',color:'#755039',bold:true,uppercase:true,align:'center',letter:1},info_panel:{x:10,y:54,w:80,h:39,size:7,font:'Space Grotesk',color:'#3e261b',align:'left',letter:.03},batch_no:{x:16,y:82,w:22,size:7,font:'Space Grotesk',color:'#3c261c',align:'center',letter:.8},roast_date:{x:39,y:82,w:22,size:7,font:'Space Grotesk',color:'#3c261c',align:'center',letter:.8},expiry_date:{x:62,y:82,w:22,size:7,font:'Space Grotesk',color:'#3c261c',align:'center',letter:.8},description:{x:14,y:94,w:72,size:6,font:'Jost',color:'#6a4937',align:'center',letter:.25}}};
...
function apply(n){const s=state.blocks[n]||{}; if(n==='info_panel'){const p=document.querySelector('.label-info-panel[data-block="info_panel"]');if(!p)return;p.style.left=(s.x||10)+'%';p.style.top=(s.y||54)+'%';p.style.right='auto';p.style.bottom='auto';p.style.width=(s.w||80)+'%';p.style.height=(s.h||39)+'%';p.style.fontFamily='"'+(s.font||'Space Grotesk')+'",sans-serif';p.style.color=s.color||'#3e261b';p.style.letterSpacing=(s.letter||.03)+'px';p.classList.toggle('active',n===active);return} if(n==='logo'){const lg=document.querySelector('.label-logo[data-block="logo"]'); if(!lg)return; lg.style.left=(s.x||0)+'%'; lg.style.top=(s.y||0)+'%'; lg.style.width=(s.w||20)+'%'; lg.style.height='auto'; lg.classList.toggle('active',n===active); return} const el=document.querySelector('.label-text[data-block="'+n+'"]'); if(!el)return; el.textContent=s.uppercase?val(n).toUpperCase():val(n); el.style.left=(s.x||0)+'%'; el.style.top=(s.y||0)+'%'; el.style.width=(s.w||50)+'%'; el.style.fontSize=(s.size||12)+'px'; el.style.fontFamily='"'+(s.font||'Jost')+'",sans-serif'; el.style.color=s.color||'#2c1711'; el.style.fontWeight=s.bold?'900':'500'; el.style.fontStyle=s.italic?'italic':'normal'; el.style.textAlign=s.align||'left'; el.style.letterSpacing=(s.letter||0)+'px'; el.classList.toggle('active',n===active)}
...
function load(){const s=state.blocks[active]||{};c.fontFamily.value=s.font||(active==='info_panel'?'Space Grotesk':'Jost');c.fontColor.value=s.color||(active==='info_panel'?'#3e261b':'#7d1720');c.fontSize.value=s.size||(active==='info_panel'?7:12);c.posX.value=s.x||0;c.posY.value=s.y||0;c.blockWidth.value=s.w||(active==='info_panel'?80:50);c.panelHeight.value=s.h||(active==='info_panel'?39:28);c.letterSpacing.value=s.letter||0;outs();refresh()}
...
blockSelect.addEventListener('change',function(){active=this.value;load()});document.querySelectorAll('.label-text,.label-logo,.label-info-panel').forEach(e=>e.addEventListener('click',function(){active=this.dataset.block;blockSelect.value=active;load()}));
