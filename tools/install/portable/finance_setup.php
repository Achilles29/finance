...
try {
    ...
    $action=$argv[1]??'';
    if(!in_array($action,['check','prepare','run','sync','retry-input'],true)||count($argv)!==2)throw new RuntimeException('USE_CHECK_PREPARE_RUN_SYNC_OR_RETRY_INPUT');
    if($action==='retry-input')$action='retryInput';
    $result=$action==='check'?PortableInstaller::requirements():$installer->$action();
