...
$to=array_replace($from,['version'=>'0.1.0-alpha.22','source_commit'=>str_repeat('b',40),'release_manifest_sha256'=>str_repeat('f',64),'distribution_profile_version'=>10]);
...
$p=['purpose'=>UpdateAuthorization::PURPOSE,'product_code'=>'NAMUA_FINANCE','authorization_id'=>'00000000-0000-4000-8000-000000000022','nonce'=>bin2hex(random_bytes(32)),'issued_at'=>1800000000,'expires_at'=>1800003600,'identity'=>$identity,'machine_fingerprint_sha256'=>$from['machine_fingerprint_sha256'],'from'=>UpdateAuthorization::releaseBinding($from),'to'=>UpdateAuthorization::releaseBinding($to),'migration_policy'=>'upgrade'];
...
echo json_encode(['status'=>'PASS','checks'=>$n,'contract'=>'PROPOSAL_NOT_LIVE_CONTROL','apply_supported'=>false,'database_accessed'=>false])."\n";
