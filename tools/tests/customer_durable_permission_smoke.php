...
foreach([8,9]as$version)foreach([1800000000,1900000000,2000000000]as$at){PortablePackage::assertPermissionWindow($p,$version,true,$at);$check(true,'Signed durable window has no installation deadline');$check(!SetupUi::permissionExpired($p+['profile_version'=>$version],$at),'UI agrees after years of waiting');}
...
foreach([6,7,10]as$v)$reject($p,$v,'SETUP_PERMISSION_BINDING_INVALID');
