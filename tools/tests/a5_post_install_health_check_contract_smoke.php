...
$check(($release['baseline']['ok'] ?? false) === true && count($release['catalog']['migrations'] ?? []) === 6, 'release manifest binds the canonical baseline, seeds, catalog, and health checker');
...
while (($line = fgets(STDIN)) !== false) {
    if (strpos($line, '__A513_TABLE__') !== false) {
        ...
        echo "__A513_LEDGER_COUNT__\t" . ($mode === 'ledger_count' ? '99' : ($policy === 'clean_install' ? '6' : '5')) . "\n";
...
$check(($upgrade['migration_ledger_rows'] ?? null) === 5 && ($upgrade['reference_seed'] ?? '') === 'preserved_customer_state', 'upgrade health checks release files, required schema, exact ledger, RBAC, and owner without replacing customer seed');

$clean = a513_check_database($release, 'clean_install', $option, 'a513_clean_ok');
$check(($clean['migration_ledger_rows'] ?? null) === 6 && ($clean['reference_seed'] ?? '') === 'exact', 'clean-install health additionally checks exact reference seed and Telegram safe default');
