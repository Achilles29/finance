...
$check(substr_count((string)$source,"'--database-name-file='.")===3&&strpos((string)$source,"['applied'] ?? null) !== 9")!==false&&strpos((string)$source,"['skipped'] ?? null) !== 9")!==false,'both migration runs and fingerprint use the separate database-name file with nine-migration apply/replay counts');
$check(strpos((string)$source,'LOWER(HEX(migration_id))')!==false&&strpos((string)$source,"'2026-09-06d-pos-mobile-reprint-step-up'")!==false&&strpos((string)$source,"bin2hex(\$managedMigration['id']) . \"\\t\" . \$managedMigration['sha256']")!==false&&strpos((string)$source,'$actualRegistryRows !== $expectedRegistryRows')!==false,'registry verification binds all nine ordered upgrade IDs to exact catalog checksums');
$check(strpos((string)$source,'$registryRows = 9;')!==false&&strpos((string)$source,"'state'=>\$registryRows === 9 ? 'COMPATIBLE_V1' : 'not_verified'")!==false&&strpos((string)$source,"'bootstrap_rows'=>\$registryRows")!==false,'registry evidence reports exactly nine verified upgrade rows');
