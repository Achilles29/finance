...
const clicks = [];
let response = {ok:true,json:async()=>({ok:true,message:'Masuk antrean'})};
let calls = 0;
let allow = true;
let lastRequest;
let notice;
...
(async()=>{
  ...
  allow=true;
  let resolve;
  response={ok:true,json:()=>new Promise(r=>{resolve=r;})};
  const first=clicks[0](event);
  await Promise.resolve();
  assert.equal(button.disabled,true);
  await clicks[0](event);assert.equal(calls,1);
  resolve({ok:true,message:'Masuk antrean'});await first;
  ...
  response={ok:false,json:async()=>({message:'<img src=x onerror=alert(1)>'})};
  ...
  assert.match(notice.textContent,/status antrean belum pasti/);assert.equal(button.disabled,false);
  response={ok:false,json:async()=>{throw new Error('HTML login');}};await clicks[0](event);
