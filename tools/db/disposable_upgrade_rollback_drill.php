...
function a513r_run(string $bundleDir, string $adminOption, string $evidenceDir, string $releaseRoot, string $releaseManifest): array
{
    ...
    try {
        ...
        if (($upgrade['applied'] ?? null) !== 7 || ($upgrade['skipped'] ?? null) !== 0) a513r_fail('upgrade_apply', 'Disposable upgrade did not apply the exact release plan.');
        $phases['upgrade'] = true;
        $health = a513_check_database($release, 'upgrade', $targetOption, $target);
        if (($health['status'] ?? '') !== 'ok' || ($health['migration_ledger_rows'] ?? null) !== 7) a513r_fail('health_after_upgrade', 'Post-upgrade health check did not pass.');
