...
-- Repair ulang BERAS KITCHEN Reguler Juli 2026 setelah ada adjustment/log-repair baru.
--
-- Target final:
-- - monthly stock BERAS KITCHEN Reguler = 0
-- - LOT FIFO BERAS KITCHEN Reguler bulan Juli = 0 / CLOSED
-- - total movement bulan Juli per profile = 0
--
-- Catatan:
-- Histori adjustment/log yang sudah terjadi tidak dihapus. Script ini membuat/koreksi
-- satu movement manual_repair_beras_zero per profile sebagai balancing audit.
...
CREATE TABLE IF NOT EXISTS zz_bak_div_monthly_20260726c_beras_zero AS
SELECT *
FROM inv_division_monthly_stock
WHERE 1 = 0;

INSERT INTO zz_bak_div_monthly_20260726c_beras_zero
SELECT *
FROM inv_division_monthly_stock
...
CREATE TABLE IF NOT EXISTS zz_bak_material_fifo_lot_20260726c_beras_zero AS
SELECT *
FROM inv_material_fifo_lot
WHERE 1 = 0;

INSERT INTO zz_bak_material_fifo_lot_20260726c_beras_zero
SELECT *
FROM inv_material_fifo_lot
...
CREATE TABLE IF NOT EXISTS zz_bak_stock_movement_20260726c_beras_zero AS
SELECT *
FROM inv_stock_movement_log
WHERE 1 = 0;

INSERT INTO zz_bak_stock_movement_20260726c_beras_zero
SELECT *
FROM inv_stock_movement_log
...
CREATE TABLE IF NOT EXISTS zz_bak_inv_stock_adj_20260726c_beras_zero AS
SELECT h.*
FROM inv_stock_adjustment h
WHERE 1 = 0;

INSERT INTO zz_bak_inv_stock_adj_20260726c_beras_zero
SELECT DISTINCT h.*
FROM inv_stock_adjustment h
JOIN inv_stock_adjustment_line l ON l.adjustment_id = h.id
WHERE h.stock_scope = 'DIVISION'
  AND h.division_id = @division_id
  AND h.destination_type = @destination_type
  AND h.adjustment_date BETWEEN @month_key AND LAST_DAY(@month_key)
  AND l.item_id = @item_id
  AND l.material_id = @material_id;

CREATE TABLE IF NOT EXISTS zz_bak_inv_stock_adj_line_20260726c_beras_zero AS
SELECT l.*
FROM inv_stock_adjustment_line l
WHERE 1 = 0;

INSERT INTO zz_bak_inv_stock_adj_line_20260726c_beras_zero
SELECT l.*
FROM inv_stock_adjustment h
JOIN inv_stock_adjustment_line l ON l.adjustment_id = h.id
WHERE h.stock_scope = 'DIVISION'
...
LEFT JOIN inv_stock_movement_log ml
 ...
 AND COALESCE(ml.ref_table, '') <> 'manual_repair_beras_zero'
...
SELECT
  ...
  'Manual repair 2026-07-26c: reset BERAS KITCHEN regular to zero after adjustment/log repair' AS notes,
  NULL AS created_by,
  NOW() AS created_at
...
SET opening_qty_buy = 0.0000,
    ...
    source_mode = 'REBUILD',
    notes = LEFT(CONCAT(COALESCE(notes, ''), ' | Repair 2026-07-26c: BERAS KITCHEN regular reset opening/closing/value/lot/movement to 0'), 255),
    updated_at = NOW()
WHERE month_key = @month_key
  AND division_id = @division_id
  AND destination_type = @destination_type
