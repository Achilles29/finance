...
-- ============================================================
-- File   : 2026-08-19b_inventory_stock_health_menu_seed.sql
-- Tujuan :
-- 1) Menambahkan halaman Kesehatan Stok Aktif ke Kontrol Stok
-- 2) Memberi akses baca kepada operator inventory dan manajemen
-- 3) Tidak mengubah stok, lot, movement, maupun saldo historis
-- ============================================================
...
VALUES (
  'inventory.stock.health.index',
  'Kesehatan Stok Aktif',
  'INVENTORY',
  'Antrean pemeriksaan stok bulan aktif terhadap lot dan nilai sebelum cut-off.',
  1
)
...
WHERE page.page_code = 'inventory.stock.health.index'
ON DUPLICATE KEY UPDATE
...
SELECT
  ...
  NOW()
FROM auth_role role
JOIN sys_page page ON page.page_code = 'inventory.stock.health.index'
...
WHERE m.menu_code = 'inventory.stock.health';
