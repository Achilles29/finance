...
**Pembaruan status eksekusi:** 2026-09-06, setelah Batch 176 menutup Void dan
Refund **POS Mobile/APK** dengan reauth password dan proof satu-kali yang
...
| A1 — security/RBAC/scope | `IN_PROGRESS` | `STAGING_PASS` | `BLOCKED` | `PARTIAL` | Multi-role/scope web-mobile terbukti fail-closed di staging; web dan POS Mobile Void/Refund, Reopen/Reprint web, Post/VOID Adjustment Base/Prepare, Adjustment Stok Gudang/Divisi, Component Batch Produksi/Daily Component, Daily Recon Component, Transfer Stok Divisi, serta seluruh writer Stock Opening telah memakai boundary CSRF/reauth pada aksi irreversible. Baseline izin per jabatan, MFA, step-up aksi mobile lain, dan UAT role/APK belum selesai. |
...
| `baseline/2026-09-05_clean_install_schema.sql` | Clean-install schema-only | `STAGING_PASS` | 285 tabel dan checksum terkunci; catalog clean-install berisi sembilan migration termasuk history/restore Formula dan proof POS Mobile. | `NOT_FOR_UPGRADE` | Hanya titik awal database customer baru, dilanjutkan migration runner policy clean_install. |

Migration runner kini mengelola sembilan file: `2026-09-04c`, clean-install-only
...
`2026-09-06c` proof reversal POS Mobile.
...
per token, dan consume atomik sebelum writer. Reprint serta aksi mobile
