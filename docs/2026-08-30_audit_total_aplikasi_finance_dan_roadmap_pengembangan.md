...
**Pembaruan status eksekusi:** 2026-09-06, setelah Batch 168 mengunci editor
massal Resep Produk dengan revision optimistic-concurrency, lock produk/baris,
dan audit before/after atomik. Simpan dari tab lama kini ditolak agar tidak
menimpa resep pengguna lain. Batch 167 menambahkan reauth password one-use
...
| A1 — security/RBAC/scope | `IN_PROGRESS` | `STAGING_PASS` | `BLOCKED` | `PARTIAL` | Multi-role/scope web-mobile terbukti fail-closed di staging; web Void/Refund/Reopen/Reprint, Post/VOID Adjustment Base/Prepare, Adjustment Stok Gudang/Divisi, Component Batch Produksi/Daily Component, Daily Recon Component, Transfer Stok Divisi, serta seluruh writer Stock Opening telah memakai boundary CSRF serta reauth pada aksi irreversible. Baseline izin per jabatan, MFA, step-up aksi lain/mobile, dan UAT role/APK belum selesai. |
...
| `AUD-A1-SEC-02` | P0-02 | P0 / A1 | Writer resep, formula, extra, dan bundle belum seragam. | Seluruh writer mempunyai RBAC aksi, CSRF/POST, concurrency, audit, dan formula versioning. | `IN_PROGRESS` | `AUTO_PASS` | `BLOCKED` | Batch 54–68 sudah memberi guard dasar. Batch 168 menutup editor massal Resep Produk: snapshot revision, `FOR UPDATE` pada produk/baris, rollback konflik, dan audit before/after atomik. Writer resep individual serta formula, extra, bundle, dan formula versioning masih terbuka. |
...
| `AUD-A1-STEP-01` | NEW-04 | P0 / A1 | Tindakan finansial sensitif hanya bergantung pada sesi login/RBAC sehingga transaksi yang ditinggal di perangkat kasir dapat dipakai ulang. | Reauth tidak mengubah matrix izin: password diverifikasi pada endpoint scoped atau form server-side, mengeluarkan proof acak satu-kali yang terikat sesi+user+aksi+target, lalu writer mengonsumsi proof sebelum model. | `IN_PROGRESS` | `AUTO_PASS` | `BLOCKED` | Batch 156: Void Kasir dan Refund Pesanan Terbayar **web** memakai password masked, CSRF transaksi, proof hash 180 detik, one-use, dan limiter kegagalan lokal sesi. Batch 157 memakai kontrak proof sama untuk Reopen Periode Keuangan web tanpa meneruskan password ke model. Batch 158 menutup Cetak Ulang Order Kasir web dan menambahkan CSRF sebelum target printer dibuat. Batch 159–160 menutup Post/VOID Adjustment Base/Prepare. Batch 161 menutup **Post/VOID Adjustment Stok Gudang dan Divisi web** dengan CSRF header scoped dan proof `STOCK_ADJUSTMENT_POST`/`STOCK_ADJUSTMENT_VOID`. Batch 162 menutup Save/Delete Draft dan Post/VOID Component Batch Produksi web dengan token `X-Production-Component-Batch-Csrf` serta proof `COMPONENT_BATCH_POST`/`COMPONENT_BATCH_VOID` sebelum `ComponentStockWriter`; jalur Quick Batch dan Quick Adjustment Daily Component mengikuti token/proof endpoint resmi. Batch 163 menutup Daily Recon Component: simpan fisik/konfirmasi memakai CSRF scoped, sedangkan quick-adjust wajib proof `COMPONENT_DAILY_RECON_POST` yang terikat user dan component sebelum membuat/post adjustment. Batch 164 menutup **Transfer Stok Divisi web**: Save/Delete Draft memakai CSRF `X-Stock-Transfer-Csrf`, sedangkan Post/VOID wajib proof `STOCK_TRANSFER_POST`/`STOCK_TRANSFER_VOID` terikat user dan dokumen sebelum writer FIFO/rollback dipanggil; auto_post ditolak. Batch 166–167 menutup seluruh writer Stock Opening: manual `STOCK_OPENING_POST` terikat scope Gudang/ID Divisi, VOID `STOCK_OPENING_VOID` terikat snapshot tepat, dan import `STOCK_OPENING_IMPORT` terikat satu divisi aktif; baris lintas divisi ditolak sebelum writer. Password tidak diteruskan ke writer. API/APK, MFA, serta UAT per role/perangkat masih terbuka. |
...
2. Lanjutkan `AUD-A1-SEC-02` secara kecil: tutup writer Resep individual,
   Formula Component, Extra, dan Bundle dengan concurrency/audit yang seragam;
   rancang kontrak APK reauth terpisah dan jangan memakai password dalam writer.
